...
<h1 align="center">🦀 YantrikClaw — Asistente Personal de IA</h1>
...
> **Configuración recomendada:** ejecuta `yantrikclaw onboard` en tu terminal. YantrikClaw Onboard te guía paso a paso en la configuración del gateway, workspace, canales y proveedor. Es la ruta de configuración recomendada y funciona en macOS, Linux y Windows (vía WSL2). ¿Nueva instalación? Empieza aquí: [Primeros pasos](#inicio-rápido)
...
`yantrikclaw onboard` se ejecuta automáticamente después de la instalación para configurar tu workspace y proveedor.
...
yantrikclaw gateway                # por defecto: 127.0.0.1:42617
yantrikclaw gateway --port 0       # puerto aleatorio (seguridad reforzada)
...
¿Actualizando? Ejecuta `yantrikclaw doctor` después de actualizar.
...
YantrikClaw puede importar tu workspace, memoria y configuración de OpenClaw:
...
Esto migra tus entradas de memoria, archivos del workspace y configuración de `~/.openclaw/` a `~/.yantrikclaw/`. La configuración se convierte de JSON a TOML automáticamente.

## Valores predeterminados de seguridad (acceso por DM)

YantrikClaw se conecta a superficies de mensajería reales. Trata los DMs entrantes como entrada no confiable.
...
- Aprobar con: `yantrikclaw pairing approve <channel> <code>` (luego el remitente se agrega a una lista de permitidos local).
- Los DMs públicos entrantes requieren una activación explícita en `config.toml`.
- Ejecuta `yantrikclaw doctor` para detectar políticas de DM riesgosas o mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **No estamos afiliados** con `openagen/yantrikclaw`, `yantrikclaw.org` ni `yantrikclaw.net`. Los dominios `yantrikclaw.org` y `yantrikclaw.net` actualmente apuntan al fork `openagen/yantrikclaw`, y ese dominio/repositorio están suplantando nuestro sitio web/proyecto oficial.                                                                                       | No confíes en información, binarios, recaudaciones de fondos o anuncios de esas fuentes. Usa solo [este repositorio](https://github.com/yantrikclaw-labs/yantrikclaw) y nuestras cuentas sociales verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Importante_ | Nuestro sitio web oficial ya está en línea: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). Gracias por tu paciencia mientras preparábamos el lanzamiento. Seguimos viendo intentos de suplantación, así que **no** te unas a ninguna actividad de inversión o recaudación que use el nombre de YantrikClaw a menos que se publique a través de nuestros canales oficiales.                            | Usa [este repositorio](https://github.com/yantrikclaw-labs/yantrikclaw) como la única fuente de verdad. Sigue [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Grupo)](https://www.facebook.com/groups/yantrikclawlabs) y [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) para actualizaciones oficiales. |
...
### Por qué los equipos eligen YantrikClaw
...
## Resumen de benchmarks (YantrikClaw vs OpenClaw, reproducible)
...
> Notas: Los resultados de YantrikClaw se miden en compilaciones release usando `/usr/bin/time -l`. OpenClaw requiere el runtime de Node.js (típicamente ~390MB de sobrecarga adicional de memoria), mientras que NanoBot requiere el runtime de Python. PicoClaw y YantrikClaw son binarios estáticos. Las cifras de RAM anteriores son de memoria en runtime; los requisitos de compilación son mayores.
...
`~/.yantrikclaw/config.toml` mínimo:
...
YantrikClaw soporta perfiles de autenticación nativos de suscripción (multi-cuenta, cifrados en reposo).

- Archivo de almacenamiento: `~/.yantrikclaw/auth-profiles.json`
- Clave de cifrado: `~/.yantrikclaw/.secret_key`
...
Raíz del workspace: `~/.yantrikclaw/workspace/` (configurable vía config).
...
Skills: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` o `SKILL.toml`.
...
yantrikclaw onboard              # Asistente de configuración guiada
yantrikclaw status               # Mostrar estado del daemon/agente
yantrikclaw doctor               # Ejecutar diagnósticos del sistema

# Gateway + daemon
yantrikclaw gateway              # Iniciar servidor gateway (127.0.0.1:42617)
yantrikclaw daemon               # Iniciar runtime autónomo completo

# Agente
yantrikclaw agent                # Modo de chat interactivo
yantrikclaw agent -m "message"   # Modo de mensaje único

# Gestión de servicios
yantrikclaw service install      # Instalar como servicio del SO (launchd/systemd)
yantrikclaw service start|stop|restart|status

# Canales
yantrikclaw channel list         # Listar canales configurados
yantrikclaw channel doctor       # Verificar salud de los canales
yantrikclaw channel bind-telegram 123456789

# Cron + programación
yantrikclaw cron list            # Listar trabajos programados
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# Memoria
yantrikclaw memory list          # Listar entradas de memoria
yantrikclaw memory get <key>     # Recuperar una memoria
yantrikclaw memory stats         # Estadísticas de memoria
...
yantrikclaw hardware discover    # Escanear dispositivos conectados
yantrikclaw peripheral list      # Listar periféricos conectados
yantrikclaw peripheral flash     # Flashear firmware al dispositivo
...
O salta los pasos anteriores e instala todo (dependencias del sistema, Rust, YantrikClaw) en un solo comando:
...
- Sigue [YantrikClaw Onboard](#inicio-rápido) para una configuración guiada.
...
YantrikClaw fue construido para el cangrejo suave 🦀, un asistente de IA rápido y eficiente. Construido por Argenis De La Rosa y la comunidad.
...
Si YantrikClaw ayuda en tu trabajo y quieres apoyar el desarrollo continuo, puedes donar aquí:
...
¿Nuevo en YantrikClaw? Busca issues etiquetados como [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — consulta nuestra [Guía de contribución](CONTRIBUTING.md#first-time-contributors) para saber cómo empezar. ¡PRs con IA/vibe-coded son bienvenidos! 🤖
...
- Nuevo `Skill` → `~/.yantrikclaw/workspace/skills/<name>/`
...
**Este es el único repositorio oficial de YantrikClaw:**

> https://github.com/yantrikclaw-labs/yantrikclaw

Cualquier otro repositorio, organización, dominio o paquete que afirme ser "YantrikClaw" o implique afiliación con YantrikClaw Labs **no está autorizado y no está afiliado con este proyecto**. Los forks no autorizados conocidos se listarán en [TRADEMARK.md](docs/maintainers/trademark.md).

Si encuentras suplantación o uso indebido de marca, por favor [abre un issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## Licencia

YantrikClaw tiene doble licencia para máxima apertura y protección de los contribuidores:
...
El nombre y logo de **YantrikClaw** son marcas registradas de YantrikClaw Labs. Esta licencia no otorga permiso para usarlos para implicar respaldo o afiliación. Ver [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos y prohibidos.
...
**YantrikClaw** — Cero sobrecarga. Cero compromisos. Despliega en cualquier lugar. Intercambia cualquier cosa. 🦀
