...
<h1 align="center">🦀 YantrikClaw — עוזר AI אישי</h1>
...
> **הגדרה מועדפת:** הרץ `yantrikclaw onboard` בטרמינל שלך. YantrikClaw Onboard מנחה אותך שלב אחר שלב בהגדרת ה-gateway, סביבת העבודה, הערוצים והספק. זהו נתיב ההגדרה המומלץ ועובד על macOS, Linux ו-Windows (דרך WSL2). התקנה חדשה? התחל כאן: [התחלה](#התחלה-מהירה)
...
`yantrikclaw onboard` רץ אוטומטית לאחר ההתקנה כדי להגדיר את סביבת העבודה והספק שלך.
...
משדרג? הרץ `yantrikclaw doctor` לאחר העדכון.
...
YantrikClaw יכול לייבא את סביבת העבודה, הזיכרון וההגדרות של OpenClaw שלך:
...
זה מעביר את רשומות הזיכרון, קבצי סביבת העבודה וההגדרות מ-`~/.openclaw/` ל-`~/.yantrikclaw/`. ההגדרות מומרות אוטומטית מ-JSON ל-TOML.

## ברירות מחדל אבטחה (גישת DM)

YantrikClaw מתחבר למשטחי הודעות אמיתיים. התייחס ל-DM נכנסים כקלט לא מהימן.
...
- אשר עם: `yantrikclaw pairing approve <channel> <code>` (ואז השולח נוסף לרשימת היתרים מקומית).
- DM נכנסים ציבוריים דורשים הסכמה מפורשת ב-`config.toml`.
- הרץ `yantrikclaw doctor` כדי לחשוף מדיניות DM מסוכנת או שגויה.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _קריטי_ | אנחנו **לא מזוהים** עם `openagen/yantrikclaw`, `yantrikclaw.org` או `yantrikclaw.net`. הדומיינים `yantrikclaw.org` ו-`yantrikclaw.net` מפנים כרגע ל-fork `openagen/yantrikclaw`, ואותו דומיין/מאגר מתחזים לאתר/פרויקט הרשמי שלנו. | אל תסמוך על מידע, בינאריים, גיוס כספים או הודעות ממקורות אלה. השתמש רק ב[מאגר זה](https://github.com/yantrikclaw-labs/yantrikclaw) ובחשבונות החברתיים המאומתים שלנו. |
| 2026-02-21 | _חשוב_ | האתר הרשמי שלנו כעת פעיל: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). תודה על הסבלנות בזמן שהכנו את ההשקה. אנחנו עדיין רואים ניסיונות התחזות, לכן **אל** תצטרפו לפעילות השקעה או גיוס כספים הטוענת לשם YantrikClaw אלא אם היא מפורסמת דרך הערוצים הרשמיים שלנו. | השתמש ב[מאגר זה](https://github.com/yantrikclaw-labs/yantrikclaw) כמקור האמת היחיד. עקוב אחרי [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs) ו-[Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) לעדכונים רשמיים. |
...
### למה צוותים בוחרים ב-YantrikClaw
...
## תמונת מצב של ביצועים (YantrikClaw מול OpenClaw, ניתן לשחזור)
...
> הערות: תוצאות YantrikClaw נמדדו על בנייות שחרור באמצעות `/usr/bin/time -l`. OpenClaw דורש סביבת ריצה Node.js (בדרך כלל ~390MB תקורת זיכרון נוספת), בעוד NanoBot דורש סביבת ריצה Python. PicoClaw ו-YantrikClaw הם בינאריים סטטיים. נתוני ה-RAM למעלה הם זיכרון סביבת ריצה; דרישות קומפילציה בזמן בנייה גבוהות יותר.
...
מינימלי `~/.yantrikclaw/config.toml`:
...
YantrikClaw תומך בפרופילי אימות מקוריים למנוי (רב-חשבוני, מוצפן במנוחה).

- קובץ אחסון: `~/.yantrikclaw/auth-profiles.json`
- מפתח הצפנה: `~/.yantrikclaw/.secret_key`
...
שורש סביבת עבודה: `~/.yantrikclaw/workspace/` (ניתן להגדרה דרך ההגדרות).
...
מיומנויות: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` או `SKILL.toml`.
...
או דלג על השלבים למעלה והתקן הכל (תלויות מערכת, Rust, YantrikClaw) בפקודה אחת:
...
- עקוב אחרי [YantrikClaw Onboard](#התחלה-מהירה) להגדרה מונחית.
...
YantrikClaw נבנה עבור ה-smooth crab 🦀, עוזר AI מהיר ויעיל. נבנה על ידי Argenis De La Rosa והקהילה.
...
אם YantrikClaw עוזר לעבודה שלך ואתה רוצה לתמוך בפיתוח המתמשך, אתה יכול לתרום כאן:
...
חדש ב-YantrikClaw? חפש בעיות עם התווית [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ראה את [מדריך התרומה](CONTRIBUTING.md#first-time-contributors) שלנו כדי להתחיל. PR של AI/vibe-coded מתקבלים בברכה! 🤖
...
- `Skill` חדש → `~/.yantrikclaw/workspace/skills/<name>/`
...
**זהו מאגר YantrikClaw הרשמי היחיד:**

> https://github.com/yantrikclaw-labs/yantrikclaw

כל מאגר, ארגון, דומיין או חבילה אחרים הטוענים להיות "YantrikClaw" או מרמזים על שיוך ל-YantrikClaw Labs הם **לא מורשים ולא מזוהים עם פרויקט זה**. פורקים לא מורשים ידועים ירשמו ב-[TRADEMARK.md](docs/maintainers/trademark.md).

אם אתה נתקל בהתחזות או שימוש לרעה בסימן מסחרי, אנא [פתח issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## רישיון

YantrikClaw מורשה ברישיון כפול לפתיחות מקסימלית והגנה על תורמים:
...
השם והלוגו של **YantrikClaw** הם סימנים מסחריים של YantrikClaw Labs. רישיון זה אינו מעניק הרשאה להשתמש בהם כדי לרמוז על תמיכה או שיוך. ראה [TRADEMARK.md](docs/maintainers/trademark.md) לשימושים מותרים ואסורים.
...
**YantrikClaw** — אפס תקורה. אפס פשרות. פרוס בכל מקום. החלף הכל. 🦀
