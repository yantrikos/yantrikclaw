...
This document is the canonical reference for channel configuration in YantrikClaw.
...
6. **Stale runtime state**: config changed but `yantrikclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.yantrikclaw/config.toml`.
...
When running `yantrikclaw channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `yantrikclaw models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

YantrikClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `yantrikclaw channel list`, `yantrikclaw channel doctor`, and `yantrikclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@yantrikclaw:matrix.example.com"   # optional, recommended for E2EE
...
YantrikClaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.yantrikclaw/state/whatsapp-web/session.db"
...
port = 6697
nickname = "yantrikclaw-bot"
username = "yantrikclaw"              # optional
channels = ["#yantrikclaw"]
allowed_users = ["*"]
...
- `YANTRIKCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `YANTRIKCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `yantrikclaw daemon` after config changes.
...
```bash
RUST_LOG=info yantrikclaw daemon 2>&1 | tee /tmp/yantrikclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/yantrikclaw.log
```
