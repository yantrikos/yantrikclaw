# YantrikClaw Config Reference (Operator-Oriented)
...
1. `YANTRIKCLAW_WORKSPACE` override (if set)
2. persisted `~/.yantrikclaw/active_workspace.toml` marker (if present)
3. default `~/.yantrikclaw/config.toml`

YantrikClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `yantrikclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `yantrikclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `yantrikclaw doctor traces --limit 20`
  - `yantrikclaw doctor traces --event tool_call_result --contains \"error\"`
  - `yantrikclaw doctor traces --id <trace-id>`
...
1. `YANTRIKCLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `YANTRIKCLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, YantrikClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.yantrikclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `yantrikclaw estop` to engage and `yantrikclaw estop resume` to clear levels.
...
- Security-first default: YantrikClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `YANTRIKCLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `YANTRIKCLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `YANTRIKCLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `YANTRIKCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `yantrikclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- YantrikClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `yantrikclaw doctor` before restart/rollout.
...
- While `yantrikclaw channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `YANTRIKCLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `YANTRIKCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
