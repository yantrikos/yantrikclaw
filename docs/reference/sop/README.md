...
- CLI `yantrikclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.yantrikclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.yantrikclaw/workspace/sops/deploy-prod/SOP.md
   ```

3. Validate and inspect definitions:

   ```bash
   yantrikclaw sop list
   yantrikclaw sop validate
   yantrikclaw sop show deploy-prod
   ```
