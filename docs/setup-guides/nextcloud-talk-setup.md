...
This guide covers native Nextcloud Talk integration for YantrikClaw.
...
Add this section in `~/.yantrikclaw/config.toml`:
...
yantrikclaw daemon
# or
yantrikclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, YantrikClaw verifies:
...
- YantrikClaw ignores bot-originated webhook events (`actorType = bots`).
- YantrikClaw ignores non-message/system events.
...
3. Confirm YantrikClaw receives and replies in the same room.
