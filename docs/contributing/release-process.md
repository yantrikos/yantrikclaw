...
- variable: `HOMEBREW_CORE_BOT_FORK_REPO` (for example `yantrikclaw-release-bot/homebrew-core`)
...
- variable: `SCOOP_BUCKET_REPO` (for example `yantrikclaw-labs/scoop-yantrikclaw`)

Workflow guardrails:

- release tag must be `vX.Y.Z` format
- Windows binary SHA256 extracted from `SHA256SUMS` release asset
- manifest pushed to `bucket/yantrikclaw.json` in the Scoop bucket repo
...
- PKGBUILD and .SRCINFO pushed to AUR `yantrikclaw` package
