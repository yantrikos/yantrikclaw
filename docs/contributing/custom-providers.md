...
YantrikClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export YANTRIKCLAW_API_KEY="your-api-key"
yantrikclaw agent
```

## llama.cpp Server (Recommended Local Setup)

YantrikClaw includes a first-class local provider for `llama-server`:
...
```bash
yantrikclaw models refresh --provider llamacpp
yantrikclaw agent -m "hello"
```

You do not need to export `YANTRIKCLAW_API_KEY=dummy` for this flow.

## SGLang Server

YantrikClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
yantrikclaw models refresh --provider sglang
yantrikclaw agent -m "hello"
```
...
YantrikClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
yantrikclaw models refresh --provider vllm
yantrikclaw agent -m "hello"
```
...
# Single message test
yantrikclaw agent -m "test message"
```
