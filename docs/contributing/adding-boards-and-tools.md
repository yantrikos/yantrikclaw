# Adding Boards and Tools — YantrikClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to YantrikClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.yantrikclaw/config.toml)
yantrikclaw peripheral add nucleo-f401re /dev/ttyACM0
yantrikclaw peripheral add arduino-uno /dev/cu.usbmodem12345
yantrikclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, YantrikClaw can index PDF files:
...
2. **Add to config** — `yantrikclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `yantrikclaw peripheral list` | List configured boards |
| `yantrikclaw peripheral add <board> <path>` | Add board (writes config) |
| `yantrikclaw peripheral flash` | Flash Arduino firmware |
| `yantrikclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `yantrikclaw hardware discover` | List USB devices |
| `yantrikclaw hardware info` | Chip info via probe-rs |
