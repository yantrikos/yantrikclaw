...
- `cargo check` / `cargo build` dừng lâu ở `Checking yantrikclaw`
...
### Không tìm thấy lệnh `yantrikclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `yantrikclaw`
...
Xác minh `~/.yantrikclaw/config.toml`:
...
- Dừng các tiến trình `yantrikclaw daemon` / `yantrikclaw channel start` thừa
