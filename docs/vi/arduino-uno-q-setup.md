# YantrikClaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy YantrikClaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

YantrikClaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `yantrikclaw peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt YantrikClaw trên Uno Q
...
# Clone yantrikclaw (hoặc scp project của bạn)
...
## Phase 4: Chạy YantrikClaw Daemon
...
**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — YantrikClaw phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (YantrikClaw xử lý tự động)

YantrikClaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo yantrikclaw):
```bash
...
### 5.3 Chạy YantrikClaw

```bash
yantrikclaw daemon --host 127.0.0.1 --port 3000
```

Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, YantrikClaw dùng `gpio_write` qua Bridge.
...
| 8 | Chỉnh sửa `~/.yantrikclaw/config.toml` (thêm Telegram bot_token) |
| 9 | `yantrikclaw daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: yantrikclaw"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/yantrikclaw` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`yantrikclaw peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. YantrikClaw dùng endpoint v4 chính xác.
