# Triển khai mạng — YantrikClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai YantrikClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | YantrikClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | YantrikClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — YantrikClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. YantrikClaw trên Raspberry Pi
...
- Channel Telegram hoạt động: YantrikClaw poll Telegram API (outbound)
...
```bash
yantrikclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `yantrikclaw tunnel` (xem tài liệu tunnel).

3. YantrikClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- YantrikClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `yantrikclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `yantrikclaw daemon`).
...
- [ ] Chạy `yantrikclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
