...
[INFO] ✓ Audit logging enabled (~/.config/yantrikclaw/audit.log)
...
```toml
# ~/.config/yantrikclaw/config.toml
...
$ yantrikclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/yantrikclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ yantrikclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ yantrikclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ yantrikclaw config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `yantrikclaw security --status`
