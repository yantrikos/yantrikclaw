...
Hướng dẫn này giải thích cách sử dụng gói Python `yantrikclaw-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của YantrikClaw sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
```python
import asyncio
from yantrikclaw_tools import create_agent, shell, file_read, file_write
...
```python
from yantrikclaw_tools import tool, create_agent
...
```python
import os
from yantrikclaw_tools.integrations import DiscordBot
...
# Single message
yantrikclaw-tools "What is the current date?"

# Interactive mode
yantrikclaw-tools -i
```

## So sánh với Rust YantrikClaw

| Khía cạnh | Rust YantrikClaw | yantrikclaw-tools |
|--------|---------------|-----------------|
