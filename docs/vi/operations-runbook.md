...
|---|---|---|
| Foreground runtime | `yantrikclaw daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `yantrikclaw gateway` | kiểm thử webhook endpoint |
| User service | `yantrikclaw service install && yantrikclaw service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `yantrikclaw doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `yantrikclaw channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `yantrikclaw status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.yantrikclaw/daemon_state.json` | file được cập nhật định kỳ |
...
4. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.yantrikclaw/config.toml`.
...
1. sao lưu `~/.yantrikclaw/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `yantrikclaw doctor`
