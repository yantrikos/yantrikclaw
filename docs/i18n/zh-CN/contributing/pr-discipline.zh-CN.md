...
YantrikClaw 拉取请求的质量、署名、隐私和交接规则。
...
- 如果不可避免需要类似身份的上下文，仅使用 YantrikClaw 范围的角色/标签（例如 `YantrikClawAgent`、`YantrikClawOperator`、`yantrikclaw_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`YantrikClawAgent`、`YantrikClawOperator`、`YantrikClawMaintainer`、`yantrikclaw_user`
    - 服务/运行时标签：`yantrikclaw_bot`、`yantrikclaw_service`、`yantrikclaw_runtime`、`yantrikclaw_node`
    - 环境标签：`yantrikclaw_project`、`yantrikclaw_workspace`、`yantrikclaw_channel`
