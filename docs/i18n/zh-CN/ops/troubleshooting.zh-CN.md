...
- `cargo check` / `cargo build` 似乎长时间卡在 `Checking yantrikclaw`
- 重复出现 `Blocking waiting for file lock on package cache` 或 `build directory`

YantrikClaw 中出现此问题的原因：
...
### 安装后找不到 `yantrikclaw` 命令

症状：

- 安装成功，但 shell 找不到 `yantrikclaw`
...
- 为该令牌仅保留一个活动运行时
- 停止额外的 `yantrikclaw daemon` / `yantrikclaw channel start` 进程
