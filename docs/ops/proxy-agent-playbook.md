...
- **Audience:** operators and maintainers running YantrikClaw in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside YantrikClaw runtime behavior.
...
### 1.1 Proxy only YantrikClaw internal traffic

1. Use scope `yantrikclaw`.
2. Set `http_proxy`/`https_proxy` or `all_proxy`.
3. Validate with `{"action":"get"}`.

Go to:

- [Section 4](#4-mode-a--proxy-only-for-yantrikclaw-internals)
...
|---|---|---|---|
| `yantrikclaw` | YantrikClaw internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for YantrikClaw Internals

Use when YantrikClaw provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.
...
- Runtime proxy is active for YantrikClaw HTTP clients.
