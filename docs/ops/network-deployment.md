# Network Deployment — YantrikClaw on Raspberry Pi and Local Network

This document covers deploying YantrikClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | YantrikClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | YantrikClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — YantrikClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. YantrikClaw on Raspberry Pi
...
- Telegram channel works: YantrikClaw polls Telegram API (outbound)
...
```bash
yantrikclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `yantrikclaw tunnel` (see tunnel docs).

3. YantrikClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- YantrikClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `yantrikclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `yantrikclaw daemon` service).
...
- [ ] Run `yantrikclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
YantrikClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `yantrikclaw` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo yantrikclaw service install
```

This creates:
- Init script: `/etc/init.d/yantrikclaw`
- Config directory: `/etc/yantrikclaw/`
- Log directory: `/var/log/yantrikclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo yantrikclaw service install` automatically prepares `/etc/yantrikclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `yantrikclaw` service user.

If no prior runtime state is available to migrate, create `/etc/yantrikclaw/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add yantrikclaw default

# Start the service
sudo rc-service yantrikclaw start

# Check status
sudo rc-service yantrikclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service yantrikclaw start` | Start the daemon |
| `sudo rc-service yantrikclaw stop` | Stop the daemon |
| `sudo rc-service yantrikclaw status` | Check service status |
| `sudo rc-service yantrikclaw restart` | Restart the daemon |
| `sudo yantrikclaw service status` | YantrikClaw status wrapper (uses `/etc/yantrikclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/yantrikclaw/access.log` |
| Errors/stderr | `/var/log/yantrikclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/yantrikclaw/error.log
```
...
sudo rc-service yantrikclaw stop
sudo rc-update del yantrikclaw default

# Remove init script
sudo yantrikclaw service uninstall
```
...
- The service runs as the `yantrikclaw:yantrikclaw` user (least privilege)
- Config must be at `/etc/yantrikclaw/config.toml` (explicit path in init script)
- If the `yantrikclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo yantrikclaw service install`
- [ ] Enable: `sudo rc-update add yantrikclaw default`
- [ ] Start: `sudo rc-service yantrikclaw start`
- [ ] Verify: `sudo rc-service yantrikclaw status`
- [ ] Check logs: `/var/log/yantrikclaw/error.log`
