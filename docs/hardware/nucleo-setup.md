# YantrikClaw on Nucleo-F401RE — Step-by-Step Guide

Run YantrikClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

YantrikClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
YantrikClaw includes everything for Nucleo-F401RE:
...
| Flash command | `yantrikclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via YantrikClaw

From the yantrikclaw repo root:
...
Add to `~/.yantrikclaw/config.toml`:
...
| 5 | `yantrikclaw daemon` or `yantrikclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `yantrikclaw peripheral list` to verify.
