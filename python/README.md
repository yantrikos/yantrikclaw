...
Python companion package for [YantrikClaw](https://github.com/yantrikclaw-labs/yantrikclaw) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install yantrikclaw-tools[discord]
```
...
# Run the CLI
yantrikclaw-tools "List files in the current directory"
...
```python
from yantrikclaw_tools import tool
...
| Feature | Rust YantrikClaw | yantrikclaw-tools |
|---------|---------------|----------------|
...
Use **Rust YantrikClaw** for production edge deployments. Use **yantrikclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
