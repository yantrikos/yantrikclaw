...
<h1 align="center">🦀 YantrikClaw — パーソナルAIアシスタント</h1>
...
> **推奨セットアップ：** ターミナルで `yantrikclaw onboard` を実行してください。YantrikClaw Onboardがゲートウェイ、ワークスペース、チャンネル、プロバイダーのセットアップをステップバイステップでガイドします。これは推奨されるセットアップパスで、macOS、Linux、Windows（WSL2経由）で動作します。新規インストール？ここから開始：[はじめに](#クイックスタートtldr)
...
`yantrikclaw onboard` はインストール後に自動的に実行され、ワークスペースとプロバイダーを設定します。
...
yantrikclaw gateway                # デフォルト：127.0.0.1:42617
yantrikclaw gateway --port 0       # ランダムポート（セキュリティ強化）
...
アップグレード？更新後に `yantrikclaw doctor` を実行してください。
...
YantrikClawはOpenClawのワークスペース、メモリ、設定をインポートできます：
...
これにより、メモリエントリ、ワークスペースファイル、設定が `~/.openclaw/` から `~/.yantrikclaw/` に移行されます。設定はJSONからTOMLに自動変換されます。

## セキュリティデフォルト（DMアクセス）

YantrikClawは実際のメッセージングサービスに接続します。着信DMを信頼できない入力として扱ってください。
...
- 承認方法：`yantrikclaw pairing approve <channel> <code>`（送信者がローカル許可リストに追加されます）。
- パブリック着信DMには `config.toml` での明示的なオプトインが必要です。
- `yantrikclaw doctor` を実行してリスクのある、または設定ミスのあるDMポリシーを検出します。
...
| 日付 (UTC) | レベル | 通知 | 対応 |
| ---------- | ------ | ---- | ---- |
| 2026-02-19 | _重大_ | 当プロジェクトは `openagen/yantrikclaw`、`yantrikclaw.org`、`yantrikclaw.net` とは**一切関係ありません**。`yantrikclaw.org` と `yantrikclaw.net` ドメインは現在 `openagen/yantrikclaw` フォークを指しており、そのドメイン/リポジトリは当プロジェクトの公式ウェブサイト/プロジェクトを偽装しています。 | それらのソースからの情報、バイナリ、資金調達、告知を信頼しないでください。[このリポジトリ](https://github.com/yantrikclaw-labs/yantrikclaw)と認証済みのソーシャルアカウントのみを使用してください。 |
| 2026-02-21 | _重要_ | 公式ウェブサイトが公開されました：[yantrikclawlabs.ai](https://yantrikclawlabs.ai)。ローンチ準備中のお待ちいただき、ありがとうございます。引き続き偽装行為が確認されていますので、公式チャンネルを通じて公開されない限り、YantrikClawの名前を使った投資や資金調達活動には**参加しないでください**。 | [このリポジトリ](https://github.com/yantrikclaw-labs/yantrikclaw)を唯一の信頼できる情報源として使用してください。公式アップデートは [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21)、[Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs)、[Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) をフォローしてください。 |
...
## ベンチマークスナップショット（YantrikClaw vs OpenClaw、再現可能）
...
> 注意：YantrikClawの結果はリリースビルドで `/usr/bin/time -l` を使用して測定されています。OpenClawにはNode.jsランタイム（通常約390MBの追加メモリオーバーヘッド）が必要で、NanoBotにはPythonランタイムが必要です。PicoClawとYantrikClawは静的バイナリです。上記のRAM数値はランタイムメモリです。ビルド時のコンパイル要件はより高くなります。
...
YantrikClawはサブスクリプションネイティブ認証プロファイル（マルチアカウント、保存時暗号化）をサポートしています。

- ストアファイル：`~/.yantrikclaw/auth-profiles.json`
- 暗号化キー：`~/.yantrikclaw/.secret_key`
...
ワークスペースルート：`~/.yantrikclaw/workspace/`（設定で変更可能）。
...
スキル：`~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` または `SKILL.toml`。
...
```bash
# ワークスペース管理
yantrikclaw onboard              # ガイド付きセットアップウィザード
yantrikclaw status               # デーモン/エージェントのステータス表示
yantrikclaw doctor               # システム診断を実行

# ゲートウェイ + デーモン
yantrikclaw gateway              # ゲートウェイサーバーを起動（127.0.0.1:42617）
yantrikclaw daemon               # フル自律ランタイムを起動

# エージェント
yantrikclaw agent                # インタラクティブチャットモード
yantrikclaw agent -m "message"   # 単一メッセージモード

# サービス管理
yantrikclaw service install      # OSサービスとしてインストール（launchd/systemd）
yantrikclaw service start|stop|restart|status

# チャンネル
yantrikclaw channel list         # 設定済みチャンネルの一覧
yantrikclaw channel doctor       # チャンネルヘルスの確認
yantrikclaw channel bind-telegram 123456789

# Cron + スケジューリング
yantrikclaw cron list            # スケジュールタスクの一覧
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# メモリ
yantrikclaw memory list          # メモリエントリの一覧
yantrikclaw memory get <key>     # メモリの取得
yantrikclaw memory stats         # メモリ統計
...
# ハードウェア周辺機器
yantrikclaw hardware discover    # 接続デバイスのスキャン
yantrikclaw peripheral list      # 接続周辺機器の一覧
yantrikclaw peripheral flash     # デバイスへのファームウェア書き込み
...
または、上記のステップをスキップして、単一コマンドですべてをインストール（システム依存、Rust、YantrikClaw）：
...
- [YantrikClaw Onboard](#クイックスタートtldr)でガイド付きセットアップ。
...
YantrikClawはsmooth crab 🦀のために構築されました。高速で効率的なAIアシスタント。Argenis De La Rosaとコミュニティによって構築されました。
...
YantrikClawがあなたの仕事に役立ち、継続的な開発を支援したい場合は、こちらから寄付できます：
...
YantrikClaw初心者ですか？[`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルの付いた課題を探してください — 始め方は[貢献ガイド](CONTRIBUTING.md#first-time-contributors)を参照。AI/vibe-coded PRも歓迎します！🤖
...
**これがYantrikClawの唯一の公式リポジトリです：**

> https://github.com/yantrikclaw-labs/yantrikclaw

「YantrikClaw」を名乗る、またはYantrikClaw Labsとの提携を示唆する他のリポジトリ、組織、ドメイン、パッケージは**無許可であり、本プロジェクトとは無関係です**。既知の無許可フォークは [TRADEMARK.md](docs/maintainers/trademark.md) に記載されます。

偽装や商標の悪用を見つけた場合は、[issueを作成](https://github.com/yantrikclaw-labs/yantrikclaw/issues)してください。

---

## ライセンス

YantrikClawは最大限のオープン性と貢献者保護のためにデュアルライセンスです：
...
**YantrikClaw** の名称とロゴはYantrikClaw Labsの商標です。このライセンスは、推薦や提携を暗示するための使用許可を付与しません。許可された使用と禁止された使用については [TRADEMARK.md](docs/maintainers/trademark.md) を参照してください。
...
**YantrikClaw** — ゼロオーバーヘッド。ゼロ妥協。どこでもデプロイ。何でも交換。🦀
