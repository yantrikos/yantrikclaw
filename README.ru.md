...
<h1 align="center">🦀 YantrikClaw — Персональный ИИ-ассистент</h1>
...
<p align="center">
  <a href="https://yantrikclawlabs.ai">Веб-сайт</a> ·
...
`yantrikclaw onboard` запускается автоматически после установки для настройки рабочего пространства и провайдера.
...
Обновляетесь? Выполните `yantrikclaw doctor` после обновления.
...
YantrikClaw может импортировать ваше рабочее пространство, память и конфигурацию OpenClaw:
...
Это переносит ваши записи памяти, файлы рабочего пространства и конфигурацию из `~/.openclaw/` в `~/.yantrikclaw/`. Конфигурация автоматически конвертируется из JSON в TOML.

## Настройки безопасности по умолчанию (доступ через ЛС)

YantrikClaw подключается к реальным поверхностям обмена сообщениями. Относитесь к входящим ЛС как к ненадёжному вводу.
...
- Одобрение через: `yantrikclaw pairing approve <channel> <code>` (затем отправитель добавляется в локальный список разрешённых).
- Публичные входящие ЛС требуют явного включения в `config.toml`.
- Выполните `yantrikclaw doctor` для выявления рискованных или неправильно настроенных политик ЛС.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критический_ | Мы **не связаны** с `openagen/yantrikclaw`, `yantrikclaw.org` или `yantrikclaw.net`. Домены `yantrikclaw.org` и `yantrikclaw.net` в настоящее время указывают на форк `openagen/yantrikclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт/проект. | Не доверяйте информации, бинарным файлам, сбору средств или объявлениям из этих источников. Используйте только [этот репозиторий](https://github.com/yantrikclaw-labs/yantrikclaw) и наши верифицированные аккаунты в социальных сетях. |
| 2026-02-21 | _Важный_ | Наш официальный сайт теперь доступен: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). Спасибо за терпение, пока мы готовили запуск. Мы по-прежнему видим попытки имитации, поэтому **не** присоединяйтесь к каким-либо инвестиционным или фандрайзинговым активностям, использующим имя YantrikClaw, если они не опубликованы через наши официальные каналы. | Используйте [этот репозиторий](https://github.com/yantrikclaw-labs/yantrikclaw) как единственный источник истины. Следите за обновлениями в [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs) и [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/). |
...
### Почему команды выбирают YantrikClaw
...
## Снимок бенчмарков (YantrikClaw vs OpenClaw, воспроизводимый)
...
> Примечания: результаты YantrikClaw измерены на релизных сборках с использованием `/usr/bin/time -l`. OpenClaw требует среду выполнения Node.js (обычно ~390 МБ дополнительных накладных расходов памяти), а NanoBot требует среду выполнения Python. PicoClaw и YantrikClaw — статические бинарные файлы. Показатели ОЗУ выше — это память времени выполнения; требования к компиляции при сборке выше.
...
Минимальный `~/.yantrikclaw/config.toml`:
...
YantrikClaw поддерживает нативные профили аутентификации по подписке (мультиаккаунт, шифрование в состоянии покоя).

- Файл хранилища: `~/.yantrikclaw/auth-profiles.json`
- Ключ шифрования: `~/.yantrikclaw/.secret_key`
...
Корень рабочего пространства: `~/.yantrikclaw/workspace/` (настраивается через конфигурацию).
...
Навыки: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` или `SKILL.toml`.
...
Или пропустите шаги выше и установите всё (системные зависимости, Rust, YantrikClaw) одной командой:
...
- Следуйте [YantrikClaw Onboard](#быстрый-старт) для управляемой настройки.
...
YantrikClaw был создан для smooth crab 🦀 — быстрого и эффективного ИИ-ассистента. Создан Argenis De La Rosa и сообществом.

- [yantrikclawlabs.ai](https://yantrikclawlabs.ai)
- [@yantrikclawlabs](https://x.com/yantrikclawlabs)

## Поддержите YantrikClaw

Если YantrikClaw помогает вашей работе и вы хотите поддержать дальнейшую разработку, вы можете пожертвовать здесь:
...
Новичок в YantrikClaw? Ищите задачи с меткой [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — см. наше [Руководство по участию](CONTRIBUTING.md#first-time-contributors) для начала. AI/vibe-coded PR приветствуются! 🤖
...
- Новый `Skill` → `~/.yantrikclaw/workspace/skills/<name>/`
...
**Это единственный официальный репозиторий YantrikClaw:**

> https://github.com/yantrikclaw-labs/yantrikclaw

Любой другой репозиторий, организация, домен или пакет, претендующий на звание «YantrikClaw» или подразумевающий связь с YantrikClaw Labs, является **неавторизованным и не связанным с этим проектом**. Известные неавторизованные форки будут перечислены в [TRADEMARK.md](docs/maintainers/trademark.md).

Если вы столкнётесь с имитацией или неправомерным использованием товарного знака, пожалуйста, [откройте issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## Лицензия

YantrikClaw распространяется под двойной лицензией для максимальной открытости и защиты участников:
...
Название и логотип **YantrikClaw** являются товарными знаками YantrikClaw Labs. Эта лицензия не предоставляет разрешения на их использование для подразумевания одобрения или принадлежности. См. [TRADEMARK.md](docs/maintainers/trademark.md) для разрешённых и запрещённых использований.
...
**YantrikClaw** — Нулевые накладные расходы. Нулевые компромиссы. Развёртывайте где угодно. Заменяйте что угодно. 🦀
