...
const OTP_ISSUER: &str = "YantrikClaw";
...
impl OtpValidator {
    pub fn from_config(
        config: &OtpConfig,
        yantrikclaw_dir: &Path,
        store: &SecretStore,
    ) -> Result<(Self, Option<String>)> {
        let secret_path = secret_file_path(yantrikclaw_dir);
...
pub fn secret_file_path(yantrikclaw_dir: &Path) -> PathBuf {
    yantrikclaw_dir.join(OTP_SECRET_FILE)
}
