...
// in `~/.yantrikclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.yantrikclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(yantrikclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: yantrikclaw_dir.join(".secret_key"),
            enabled,
        }
    }
