...
/// - `X-YantrikClaw-Timestamp` — unix epoch seconds
/// - `X-YantrikClaw-Nonce` — random UUID v4
/// - `X-YantrikClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-YantrikClaw-Timestamp", timestamp.to_string())
            .header("X-YantrikClaw-Nonce", &nonce)
            .header("X-YantrikClaw-Signature", &signature)
