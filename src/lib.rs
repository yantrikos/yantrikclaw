...
Examples:
  yantrikclaw gateway start              # use config defaults
  yantrikclaw gateway start -p 8080      # listen on port 8080
  yantrikclaw gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  yantrikclaw gateway start -p 0         # random available port")]
    Start {
...
Examples:
  yantrikclaw gateway restart            # restart with config defaults
  yantrikclaw gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  yantrikclaw gateway get-paircode       # show current pairing code
  yantrikclaw gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  ...
  yantrikclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  yantrikclaw channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  yantrikclaw channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `YantrikClaw` workspace
    Openclaw {
...
Examples:
  yantrikclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  yantrikclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  yantrikclaw cron add-every 60000 'Ping heartbeat'     # every minute
  yantrikclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  yantrikclaw cron once 30m 'Run backup in 30 minutes'
  yantrikclaw cron once 2h 'Follow up on deployment'
  yantrikclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  yantrikclaw cron update <task-id> --expression '0 8 * * *'
  yantrikclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  yantrikclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  yantrikclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  yantrikclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash YantrikClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash YantrikClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  yantrikclaw peripheral flash
  yantrikclaw peripheral flash --port /dev/cu.usbmodem12345
  yantrikclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash YantrikClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
