//! Wraps a discovered MCP tool as a yantrikclaw [`Tool`] so it is dispatched
...
/// A yantrikclaw [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // YantrikClaw's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // YantrikClaw's security model injects `approved: bool` into built-in tool args.
