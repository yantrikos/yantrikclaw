...
/// Top-level YantrikClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `YANTRIKCLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.yantrikclaw/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `YANTRIKCLAW_API_KEY` or `API_KEY` env vars.
    ...
    ///
    /// Can also be set via `YANTRIKCLAW_EXTRA_HEADERS` environment variable using
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `YANTRIKCLAW_LOCALE`,
    ...
    /// proactive cognition (urge pipeline). YantrikClaw acts as the channel/gateway
...
///
/// When enabled, the companion binary handles LLM inference, cognitive memory,
/// tool execution, proactive triggers, and personality evolution. YantrikClaw
...
fn default_workspaces_dir() -> String {
    "~/.yantrikclaw/workspaces".to_string()
}
...
pub struct WebSearchConfig {
    ...
    /// Used when provider is "searxng". YantrikClaw ships a bundled SearXNG
...
fn default_project_intel_report_dir() -> String {
    "~/.yantrikclaw/project-reports".into()
}
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all YantrikClaw-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "yantrikclaw" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "yantrikclaw_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "yantrikclaw".
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `YANTRIKCLAW_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `YANTRIKCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
...
/// How YantrikClaw receives events from Feishu / Lark.
///
...
fn default_estop_state_file() -> String {
    "~/.yantrikclaw/estop-state.json".to_string()
}
...
///
/// When `enabled` is true, YantrikClaw validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to YantrikClaw permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to YantrikClaw tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub yantrikclaw_permissions: Vec<String>,
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to yantrikclaw dir)
...
fn default_report_output_dir() -> String {
    "~/.yantrikclaw/security-reports".into()
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        let yantrikclaw_dir = home.join(".yantrikclaw");

        Self {
            workspace_dir: yantrikclaw_dir.join("workspace"),
            config_path: yantrikclaw_dir.join("config.toml"),
            api_key: None,
            api_url: None,
            api_path: None,
...
pub(crate) fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".yantrikclaw"));
...
/// `YANTRIKCLAW_CONFIG_DIR` > `YANTRIKCLAW_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_yantrikclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_yantrikclaw_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "YANTRIKCLAW_CONFIG_DIR",
            Self::EnvWorkspace => "YANTRIKCLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_yantrikclaw_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("YANTRIKCLAW_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let yantrikclaw_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                yantrikclaw_dir.clone(),
                yantrikclaw_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("YANTRIKCLAW_WORKSPACE") {
        if !custom_workspace.is_empty() {
            let expanded = expand_tilde_path(&custom_workspace);
            let (yantrikclaw_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
            return Ok((
                yantrikclaw_dir,
                workspace_dir,
    ...
    if let Some((yantrikclaw_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_yantrikclaw_dir).await?
    {
        return Ok((
            yantrikclaw_dir,
            workspace_dir,
    ...
    Ok((
        default_yantrikclaw_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'yantrikclaw'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "YANTRIKCLAW_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `YANTRIKCLAW_EXTRA_HEADERS` environment variable value.
///
...
///
/// When the workspace is created outside of `yantrikclaw onboard` (e.g., non-tty
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am YantrikClaw, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are YantrikClaw, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (yantrikclaw_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_yantrikclaw_dir, &default_workspace_dir).await?;

        let config_path = yantrikclaw_dir.join("config.toml");

        fs::create_dir_all(&yantrikclaw_dir)
            .await
            .with_context(|| config_dir_creation_error(&yantrikclaw_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&yantrikclaw_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // 1) YANTRIKCLAW_PROVIDER always wins when set.
        // 2) YANTRIKCLAW_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("YANTRIKCLAW_PROVIDER") {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        } else if let Ok(provider) =
            std::env::var("YANTRIKCLAW_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: YANTRIKCLAW_MODEL or MODEL
        if let Ok(model) = std::env::var("YANTRIKCLAW_MODEL").or_else(|_| std::env::var("MODEL")) {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Provider HTTP timeout: YANTRIKCLAW_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("YANTRIKCLAW_PROVIDER_TIMEOUT_SECS") {
        ...
        // Extra provider headers: YANTRIKCLAW_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("YANTRIKCLAW_EXTRA_HEADERS") {
        ...
        // Workspace directory: YANTRIKCLAW_WORKSPACE
        if let Ok(workspace) = std::env::var("YANTRIKCLAW_WORKSPACE") {
        ...
        // Open-skills opt-in flag: YANTRIKCLAW_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("YANTRIKCLAW_OPEN_SKILLS_ENABLED") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str() {
                    "1" | "true" | "yes" | "on" => self.skills.open_skills_enabled = true,
                    "0" | "false" | "no" | "off" => self.skills.open_skills_enabled = false,
                    _ => tracing::warn!(
                        "Ignoring invalid YANTRIKCLAW_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }

        // Open-skills directory override: YANTRIKCLAW_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("YANTRIKCLAW_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills script-file audit override: YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str(){
                    "1" | "true" | "yes" | "on" => self.skills.allow_scripts = true,
                    "0" | "false" | "no" | "off" => self.skills.allow_scripts = false,
                    _ => tracing::warn!(
                        "Ignoring invalid YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }

        // Skills prompt mode override: YANTRIKCLAW_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("YANTRIKCLAW_SKILLS_PROMPT_MODE") {
            if !mode.trim().is_empty() {
                if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                    self.skills.prompt_injection_mode = parsed;
                } else {
                    tracing::warn!(
                        "Ignoring invalid YANTRIKCLAW_SKILLS_PROMPT_MODE (valid: full|compact)"
                    );
                }
            }
        }

        // Gateway port: YANTRIKCLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("YANTRIKCLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Allow public bind: YANTRIKCLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("YANTRIKCLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: YANTRIKCLAW_TEMPERATURE
        if let Ok(temp_str) = std::env::var("YANTRIKCLAW_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring YANTRIKCLAW_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!(
                        "Ignoring YANTRIKCLAW_TEMPERATURE={temp_str:?}: not a valid number"
                    );
                }
            }
        }

        // Reasoning override: YANTRIKCLAW_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("YANTRIKCLAW_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("YANTRIKCLAW_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("YANTRIKCLAW_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: YANTRIKCLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("YANTRIKCLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: YANTRIKCLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("YANTRIKCLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: YANTRIKCLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("YANTRIKCLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Web search max results: YANTRIKCLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("YANTRIKCLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: YANTRIKCLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("YANTRIKCLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): YANTRIKCLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("YANTRIKCLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): YANTRIKCLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("YANTRIKCLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: YANTRIKCLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("YANTRIKCLAW_STORAGE_CONNECT_TIMEOUT_SECS") {
            if let Ok(timeout_secs) = timeout_secs.parse::<u64>() {
                if timeout_secs > 0 {
        ...
        // Proxy enabled flag: YANTRIKCLAW_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("YANTRIKCLAW_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: YANTRIKCLAW_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("YANTRIKCLAW_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("YANTRIKCLAW_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("YANTRIKCLAW_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("YANTRIKCLAW_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("YANTRIKCLAW_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid YANTRIKCLAW_PROXY_SCOPE (valid: environment|yantrikclaw|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("YANTRIKCLAW_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_yantrikclaw_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (yantrikclaw_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_yantrikclaw_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = yantrikclaw_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let yantrikclaw_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::security::SecretStore::new(yantrikclaw_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.yantrikclaw");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/yantrikclaw"));
        assert!(msg.contains("/etc/yantrikclaw"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("yantrikclaw"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:yantrikclaw");
    ...
    async fn parse_extra_headers_env_with_url_value() {
        let headers =
            parse_extra_headers_env("HTTP-Referer:https://github.com/yantrikclaw-labs/yantrikclaw");
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "yantrikclaw");
    }
...
provider = "postgres"
dbURL = "postgres://postgres:postgres@localhost:5432/yantrikclaw"
schema = "public"
table = "memories"
...
"#;
        ...
        assert_eq!(
            ...
            Some("postgres://postgres:postgres@localhost:5432/yantrikclaw")
        );
...
"#;
    ...
    async fn sync_directory_handles_existing_directory() {
        let dir = std::env::temp_dir().join(format!(
            "yantrikclaw_test_sync_directory_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("yantrikclaw_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "yantrikclaw_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("yantrikclaw_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.yantrikclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
        };
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "YANTRIKCLAW_PROXY_ENABLED",
            "YANTRIKCLAW_HTTP_PROXY",
            "YANTRIKCLAW_HTTPS_PROXY",
            "YANTRIKCLAW_ALL_PROXY",
            "YANTRIKCLAW_NO_PROXY",
            "YANTRIKCLAW_PROXY_SCOPE",
            "YANTRIKCLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        std::env::set_var("YANTRIKCLAW_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("YANTRIKCLAW_API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        std::env::set_var("YANTRIKCLAW_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        std::env::remove_var("YANTRIKCLAW_PROVIDER");
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        std::env::remove_var("YANTRIKCLAW_PROVIDER");
        std::env::set_var("YANTRIKCLAW_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        std::env::remove_var("YANTRIKCLAW_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        std::env::set_var("YANTRIKCLAW_OPEN_SKILLS_ENABLED", "true");
        std::env::set_var("YANTRIKCLAW_OPEN_SKILLS_DIR", "/tmp/open-skills");
        std::env::set_var("YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS", "yes");
        std::env::set_var("YANTRIKCLAW_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        std::env::remove_var("YANTRIKCLAW_OPEN_SKILLS_ENABLED");
        std::env::remove_var("YANTRIKCLAW_OPEN_SKILLS_DIR");
        std::env::remove_var("YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS");
        std::env::remove_var("YANTRIKCLAW_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        std::env::set_var("YANTRIKCLAW_OPEN_SKILLS_ENABLED", "maybe");
        std::env::set_var("YANTRIKCLAW_SKILLS_ALLOW_SCRIPTS", "maybe");
        std::env::set_var("YANTRIKCLAW_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        std::env::set_var("YANTRIKCLAW_PROVIDER", "openrouter");
    ...
    async fn env_override_model() {
        ...
        std::env::set_var("YANTRIKCLAW_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        std::env::remove_var("YANTRIKCLAW_MODEL");
    }
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("yantrikclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".yantrikclaw").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        std::env::set_var("HOME", &temp_home);
        std::env::set_var("YANTRIKCLAW_WORKSPACE", &workspace_dir);
        ...
        std::env::remove_var("YANTRIKCLAW_WORKSPACE");
    ...
    async fn env_override_workspace() {
        ...
        std::env::set_var("YANTRIKCLAW_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        std::env::set_var("YANTRIKCLAW_CONFIG_DIR", &explicit_config_dir);
        std::env::remove_var("YANTRIKCLAW_WORKSPACE");
        ...
        std::env::remove_var("YANTRIKCLAW_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("yantrikclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".yantrikclaw").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".yantrikclaw");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("yantrikclaw_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".yantrikclaw");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        persist_active_workspace_config_dir(&marker_config_dir)
            .await
            .unwrap();
        std::env::set_var("YANTRIKCLAW_WORKSPACE", &env_workspace_dir);
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("yantrikclaw_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".yantrikclaw");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("YANTRIKCLAW_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        std::env::set_var("YANTRIKCLAW_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("YANTRIKCLAW_GATEWAY_PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        std::env::set_var("YANTRIKCLAW_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("YANTRIKCLAW_GATEWAY_HOST");
    }
    ...
    async fn env_override_temperature() {
        ...
        std::env::set_var("YANTRIKCLAW_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        std::env::remove_var("YANTRIKCLAW_TEMPERATURE");
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("YANTRIKCLAW_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        std::env::set_var("YANTRIKCLAW_REASONING_ENABLED", "false");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        std::env::set_var("YANTRIKCLAW_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        std::env::remove_var("YANTRIKCLAW_REASONING_ENABLED");
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        std::env::set_var("YANTRIKCLAW_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        std::env::set_var("YANTRIKCLAW_REASONING_EFFORT", "HIGH");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        std::env::remove_var("YANTRIKCLAW_REASONING_EFFORT");
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        std::env::set_var("YANTRIKCLAW_CODEX_REASONING_EFFORT", "minimal");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        std::env::remove_var("YANTRIKCLAW_CODEX_REASONING_EFFORT");
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        std::env::set_var("YANTRIKCLAW_STORAGE_PROVIDER", "postgres");
        std::env::set_var("YANTRIKCLAW_STORAGE_DB_URL", "postgres://example/db");
        std::env::set_var("YANTRIKCLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        std::env::remove_var("YANTRIKCLAW_STORAGE_PROVIDER");
        std::env::remove_var("YANTRIKCLAW_STORAGE_DB_URL");
        std::env::remove_var("YANTRIKCLAW_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        std::env::set_var("YANTRIKCLAW_PROXY_ENABLED", "true");
        std::env::set_var("YANTRIKCLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var(
            "YANTRIKCLAW_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        std::env::set_var("YANTRIKCLAW_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        std::env::set_var("YANTRIKCLAW_PROXY_ENABLED", "true");
        std::env::set_var("YANTRIKCLAW_PROXY_SCOPE", "environment");
        std::env::set_var("YANTRIKCLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var("YANTRIKCLAW_HTTPS_PROXY", "http://127.0.0.1:7891");
        std::env::set_var("YANTRIKCLAW_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.yantrikclaw/estop-state.json"
require_otp_to_resume = true
"#,
        );
    ...
    async fn channel_secret_telegram_bot_token_roundtrip() {
        let dir = std::env::temp_dir().join(format!(
            "yantrikclaw_test_tg_bot_token_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn nevis_client_secret_encrypt_decrypt_roundtrip() {
        let dir = std::env::temp_dir().join(format!(
            "yantrikclaw_test_nevis_secret_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
