...
impl TelegramChannel {
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = yantrikclaw_dir.join("workspace");
        Ok(config)
    }
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to YantrikClaw now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `yantrikclaw` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@yantrikclaw_bot 654321"),
            Some("654321")
        );
    }
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let msg = serde_json::json!({
            ...
            "reply_to_message": {
                "message_id": message_id,
                "from": { "username": "yantrikclaw_user" },
