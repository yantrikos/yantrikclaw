...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("yantrikclaw_agent_starts_total", "Total agent invocations"),
            &["provider", "model"],
        )
        .expect("valid metric");
        ...
        let tokens_output_total = IntCounterVec::new(
            prometheus::Opts::new(
                "yantrikclaw_tokens_output_total",
        ...
        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("yantrikclaw_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");
        ...
        let cache_hits = IntCounterVec::new(
            prometheus::Opts::new("yantrikclaw_cache_hits_total", "Total response cache hits"),
            &["cache_type"],
        )
        .expect("valid metric");
        ...
        let cache_tokens_saved = IntCounterVec::new(
            prometheus::Opts::new(
                "yantrikclaw_cache_tokens_saved_total",
        ...
        let agent_duration = HistogramVec::new(
            HistogramOpts::new(
                "yantrikclaw_agent_duration_seconds",
        ...
        let tool_duration = HistogramVec::new(
            HistogramOpts::new(
                "yantrikclaw_tool_duration_seconds",
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "yantrikclaw_request_latency_seconds",
        ...
        let tokens_used = prometheus::IntGauge::new(
            "yantrikclaw_tokens_used_last",
            "Tokens used in the last request",
        )
        .expect("valid metric");

        let active_sessions = GaugeVec::new(
            prometheus::Opts::new("yantrikclaw_active_sessions", "Number of active sessions"),
            &[],
        )
        .expect("valid metric");

        let queue_depth = GaugeVec::new(
            prometheus::Opts::new("yantrikclaw_queue_depth", "Message queue depth"),
            &[],
        )
        .expect("valid metric");

        let hand_runs = IntCounterVec::new(
            prometheus::Opts::new("yantrikclaw_hand_runs_total", "Total hand runs by outcome"),
            &["hand", "success"],
        )
        .expect("valid metric");
        ...
        let hand_duration = HistogramVec::new(
            HistogramOpts::new(
                "yantrikclaw_hand_duration_seconds",
        ...
        let hand_findings = IntCounterVec::new(
            prometheus::Opts::new(
                "yantrikclaw_hand_findings_total",
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("yantrikclaw_agent_starts_total"));
        assert!(output.contains("yantrikclaw_tool_calls_total"));
        assert!(output.contains("yantrikclaw_heartbeat_ticks_total"));
        assert!(output.contains("yantrikclaw_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("yantrikclaw_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"yantrikclaw_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"yantrikclaw_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"yantrikclaw_errors_total{component="provider"} 2"#));
        assert!(output.contains(r#"yantrikclaw_errors_total{component="channels"} 1"#));
    }
    ...
    fn gauge_reflects_latest_value() {
        ...
        let output = obs.encode();
        assert!(output.contains("yantrikclaw_tokens_used_last 200"));
    }
    ...
    fn llm_response_tracks_request_count_and_tokens() {
        ...
        assert!(output.contains(
            r#"yantrikclaw_llm_requests_total{model="claude-sonnet",provider="openrouter",success="true"} 2"#
        ));
        assert!(output.contains(
            r#"yantrikclaw_tokens_input_total{model="claude-sonnet",provider="openrouter"} 300"#
        ));
        assert!(output.contains(
            r#"yantrikclaw_tokens_output_total{model="claude-sonnet",provider="openrouter"} 130"#
        ));
    }
    ...
    fn hand_events_track_runs_and_duration() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"yantrikclaw_hand_runs_total{hand="review",success="true"} 2"#));
        assert!(output.contains(r#"yantrikclaw_hand_runs_total{hand="review",success="false"} 1"#));
        assert!(output.contains(r#"yantrikclaw_hand_findings_total{hand="review"} 4"#));
        assert!(output.contains("yantrikclaw_hand_duration_seconds"));
    }
    ...
    fn hand_metrics_record_duration_and_findings() {
        ...
        let output = obs.encode();
        assert!(output.contains("yantrikclaw_hand_duration_seconds"));
        assert!(output.contains(r#"yantrikclaw_hand_findings_total{hand="scan"} 5"#));
        assert!(output.contains(r#"yantrikclaw_hand_runs_total{hand="scan",success="true"} 1"#));
        assert!(output.contains(r#"yantrikclaw_hand_runs_total{hand="scan",success="false"} 1"#));
    }
    ...
    fn llm_response_without_tokens_increments_request_only() {
        ...
        let output = obs.encode();
        assert!(output.contains(
            r#"yantrikclaw_llm_requests_total{model="llama3",provider="ollama",success="false"} 1"#
        ));
        // Token counters should not appear (no data recorded)
        assert!(!output.contains("yantrikclaw_tokens_input_total{"));
        assert!(!output.contains("yantrikclaw_tokens_output_total{"));
    }
}
