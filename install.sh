...
usage() {
  cat <<'USAGE'
YantrikClaw installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds YantrikClaw, configures your provider and API key,
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/yantrikos/yantrikclaw/releases"
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} YantrikClaw Guided Installer${RESET}"
...
run_docker_bootstrap() {
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official YantrikClaw image ($fallback_image)"
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/yantrikos/yantrikclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} YantrikClaw Installer${RESET}"
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing YantrikClaw installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing YantrikClaw installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized YantrikClaw data is persisted under:${RESET}"
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing YantrikClaw${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing YantrikClaw v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} YantrikClaw installed successfully (YantrikClaw ${INSTALLED_VERSION})!${RESET}"
else
