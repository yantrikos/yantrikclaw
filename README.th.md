...
<h1 align="center">🦀 YantrikClaw — ผู้ช่วย AI ส่วนตัว</h1>
...
> **การตั้งค่าที่แนะนำ:** รัน `yantrikclaw onboard` ในเทอร์มินัลของคุณ YantrikClaw Onboard จะแนะนำคุณทีละขั้นตอนในการตั้งค่า gateway, workspace, ช่องทาง และ provider เป็นเส้นทางการตั้งค่าที่แนะนำและใช้งานได้บน macOS, Linux และ Windows (ผ่าน WSL2) ติดตั้งใหม่? เริ่มที่นี่: [เริ่มต้นใช้งาน](#เริ่มต้นอย่างรวดเร็ว)
...
`yantrikclaw onboard` จะรันโดยอัตโนมัติหลังติดตั้งเพื่อกำหนดค่า workspace และ provider ของคุณ
...
yantrikclaw gateway                # ค่าเริ่มต้น: 127.0.0.1:42617
yantrikclaw gateway --port 0       # พอร์ตสุ่ม (ความปลอดภัยเพิ่มขึ้น)
...
กำลังอัปเกรด? รัน `yantrikclaw doctor` หลังจากอัปเดต
...
YantrikClaw สามารถนำเข้า workspace, หน่วยความจำ และการกำหนดค่าจาก OpenClaw ของคุณ:
...
สิ่งนี้จะย้ายรายการหน่วยความจำ ไฟล์ workspace และการกำหนดค่าจาก `~/.openclaw/` ไปยัง `~/.yantrikclaw/` การกำหนดค่าจะถูกแปลงจาก JSON เป็น TOML โดยอัตโนมัติ

## ค่าเริ่มต้นด้านความปลอดภัย (การเข้าถึง DM)

YantrikClaw เชื่อมต่อกับพื้นผิวการส่งข้อความจริง ถือว่า DM ขาเข้าเป็นข้อมูลที่ไม่น่าเชื่อถือ
...
- อนุมัติด้วย: `yantrikclaw pairing approve <channel> <code>` (จากนั้นผู้ส่งจะถูกเพิ่มในรายการอนุญาตในเครื่อง)
- DM ขาเข้าสาธารณะต้องมีการเลือกเข้าร่วมอย่างชัดเจนใน `config.toml`
- รัน `yantrikclaw doctor` เพื่อค้นหานโยบาย DM ที่เสี่ยงหรือกำหนดค่าผิด
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _วิกฤต_  | เรา**ไม่มีส่วนเกี่ยวข้อง**กับ `openagen/yantrikclaw`, `yantrikclaw.org` หรือ `yantrikclaw.net` โดเมน `yantrikclaw.org` และ `yantrikclaw.net` ปัจจุบันชี้ไปที่ fork `openagen/yantrikclaw` และโดเมน/repository เหล่านั้นกำลังปลอมตัวเป็นเว็บไซต์/โปรเจกต์อย่างเป็นทางการของเรา                                                                                       | อย่าเชื่อถือข้อมูล ไบนารี การระดมทุน หรือประกาศจากแหล่งเหล่านั้น ใช้เฉพาะ[repository นี้](https://github.com/yantrikclaw-labs/yantrikclaw)และบัญชีโซเชียลที่ได้รับการยืนยันของเรา                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _สำคัญ_ | เว็บไซต์อย่างเป็นทางการของเราพร้อมใช้งานแล้ว: [yantrikclawlabs.ai](https://yantrikclawlabs.ai) ขอบคุณสำหรับความอดทนขณะที่เราเตรียมการเปิดตัว เรายังคงเห็นความพยายามในการแอบอ้าง ดังนั้น**อย่า**เข้าร่วมกิจกรรมการลงทุนหรือระดมทุนที่อ้างชื่อ YantrikClaw เว้นแต่จะเผยแพร่ผ่านช่องทางอย่างเป็นทางการของเรา                            | ใช้[repository นี้](https://github.com/yantrikclaw-labs/yantrikclaw)เป็นแหล่งข้อมูลที่เชื่อถือได้เพียงแหล่งเดียว ติดตาม [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs) และ [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) สำหรับอัปเดตอย่างเป็นทางการ |
...
## สรุป Benchmark (YantrikClaw vs OpenClaw, ทำซ้ำได้)
...
> หมายเหตุ: ผลลัพธ์ YantrikClaw วัดจาก release builds โดยใช้ `/usr/bin/time -l` OpenClaw ต้องการ runtime Node.js (โดยทั่วไป ~390MB overhead หน่วยความจำเพิ่มเติม) ในขณะที่ NanoBot ต้องการ runtime Python PicoClaw และ YantrikClaw เป็นไบนารีแบบ static ตัวเลข RAM ด้านบนเป็นหน่วยความจำ runtime ความต้องการการคอมไพล์ตอน build สูงกว่า
...
ขั้นต่ำ `~/.yantrikclaw/config.toml`:
...
YantrikClaw รองรับโปรไฟล์การยืนยันตัวตนแบบ subscription-native (หลายบัญชี, เข้ารหัสเมื่อเก็บ)

- ไฟล์จัดเก็บ: `~/.yantrikclaw/auth-profiles.json`
- คีย์เข้ารหัส: `~/.yantrikclaw/.secret_key`
...
Root workspace: `~/.yantrikclaw/workspace/` (กำหนดค่าได้ผ่าน config)
...
Skills: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` หรือ `SKILL.toml`
...
```bash
# การจัดการ workspace
yantrikclaw onboard              # วิซาร์ดการตั้งค่าแบบแนะนำ
yantrikclaw status               # แสดงสถานะ daemon/เอเจนต์
yantrikclaw doctor               # รันการวินิจฉัยระบบ

# Gateway + daemon
yantrikclaw gateway              # เริ่มเซิร์ฟเวอร์ gateway (127.0.0.1:42617)
yantrikclaw daemon               # เริ่ม runtime อัตโนมัติเต็มรูปแบบ

# เอเจนต์
yantrikclaw agent                # โหมดแชทโต้ตอบ
yantrikclaw agent -m "message"   # โหมดข้อความเดียว

# การจัดการบริการ
yantrikclaw service install      # ติดตั้งเป็นบริการ OS (launchd/systemd)
yantrikclaw service start|stop|restart|status

# ช่องทาง
yantrikclaw channel list         # แสดงรายการช่องทางที่กำหนดค่า
yantrikclaw channel doctor       # ตรวจสอบสุขภาพช่องทาง
yantrikclaw channel bind-telegram 123456789

# Cron + การกำหนดเวลา
yantrikclaw cron list            # แสดงรายการงานที่กำหนดเวลา
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# หน่วยความจำ
yantrikclaw memory list          # แสดงรายการหน่วยความจำ
yantrikclaw memory get <key>     # ดึงหน่วยความจำ
yantrikclaw memory stats         # สถิติหน่วยความจำ
...
# อุปกรณ์ต่อพ่วง
yantrikclaw hardware discover    # สแกนอุปกรณ์ที่เชื่อมต่อ
yantrikclaw peripheral list      # แสดงรายการอุปกรณ์ต่อพ่วงที่เชื่อมต่อ
yantrikclaw peripheral flash     # แฟลช firmware ไปยังอุปกรณ์
...
หรือข้ามขั้นตอนด้านบนและติดตั้งทุกอย่าง (dependencies ระบบ, Rust, YantrikClaw) ในคำสั่งเดียว:
...
- ทำตาม [YantrikClaw Onboard](#เริ่มต้นอย่างรวดเร็ว) สำหรับการตั้งค่าแบบแนะนำ
...
YantrikClaw ถูกสร้างสำหรับ smooth crab 🦀 ผู้ช่วย AI ที่เร็วและมีประสิทธิภาพ สร้างโดย Argenis De La Rosa และชุมชน

- [yantrikclawlabs.ai](https://yantrikclawlabs.ai)
- [@yantrikclawlabs](https://x.com/yantrikclawlabs)

## สนับสนุน YantrikClaw

หาก YantrikClaw ช่วยงานของคุณและคุณต้องการสนับสนุนการพัฒนาต่อเนื่อง คุณสามารถบริจาคที่นี่:
...
ใหม่กับ YantrikClaw? มองหา issues ที่มีป้ายกำกับ [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — ดู[คู่มือการมีส่วนร่วม](CONTRIBUTING.md#first-time-contributors)สำหรับวิธีเริ่มต้น ยินดีรับ PR ที่สร้างด้วย AI/vibe-coded! 🤖
...
- `Skill` ใหม่ → `~/.yantrikclaw/workspace/skills/<name>/`
...
**นี่คือ repository อย่างเป็นทางการเพียงแห่งเดียวของ YantrikClaw:**

> https://github.com/yantrikclaw-labs/yantrikclaw

repository, องค์กร, โดเมน หรือแพ็กเกจอื่นใดที่อ้างว่าเป็น "YantrikClaw" หรือบ่งบอกถึงการเกี่ยวข้องกับ YantrikClaw Labs นั้น**ไม่ได้รับอนุญาตและไม่มีส่วนเกี่ยวข้องกับโปรเจกต์นี้** Fork ที่ไม่ได้รับอนุญาตที่ทราบจะถูกระบุไว้ใน [TRADEMARK.md](docs/maintainers/trademark.md)

หากคุณพบการแอบอ้างหรือการใช้เครื่องหมายการค้าในทางที่ผิด โปรด[เปิด issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues)

---

## สัญญาอนุญาต

YantrikClaw มี dual-license เพื่อความเปิดกว้างสูงสุดและการปกป้องผู้มีส่วนร่วม:
...
ชื่อและโลโก้ **YantrikClaw** เป็นเครื่องหมายการค้าของ YantrikClaw Labs สัญญาอนุญาตนี้ไม่ให้สิทธิ์ในการใช้เพื่อบ่งบอกถึงการรับรองหรือการเกี่ยวข้อง ดู [TRADEMARK.md](docs/maintainers/trademark.md) สำหรับการใช้งานที่อนุญาตและห้าม
...
**YantrikClaw** — ไม่มีโอเวอร์เฮด ไม่มีการประนีประนอม Deploy ที่ไหนก็ได้ สลับอะไรก็ได้ 🦀
