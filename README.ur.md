...
<h1 align="center">🦀 YantrikClaw — ذاتی AI اسسٹنٹ</h1>
...
> **تجویز کردہ سیٹ اپ:** اپنے ٹرمینل میں `yantrikclaw onboard` چلائیں۔ YantrikClaw Onboard آپ کو gateway، workspace، چینلز، اور provider ترتیب دینے میں مرحلہ وار رہنمائی کرتا ہے۔ یہ تجویز کردہ سیٹ اپ راستہ ہے اور macOS، Linux، اور Windows (WSL2 کے ذریعے) پر کام کرتا ہے۔ نئی تنصیب؟ یہاں سے شروع کریں: [شروع کریں](#فوری-آغاز)
...
`yantrikclaw onboard` انسٹال کے بعد خود بخود چلتا ہے تاکہ آپ کا workspace اور provider ترتیب دیا جا سکے۔
...
yantrikclaw gateway                # ڈیفالٹ: 127.0.0.1:42617
yantrikclaw gateway --port 0       # بے ترتیب پورٹ (سیکیورٹی مضبوط)
...
اپ گریڈ کر رہے ہیں؟ اپ ڈیٹ کے بعد `yantrikclaw doctor` چلائیں۔
...
YantrikClaw آپ کا OpenClaw workspace، میموری، اور کنفیگریشن درآمد کر سکتا ہے:
...
یہ آپ کے میموری اندراجات، workspace فائلیں، اور کنفیگریشن `~/.openclaw/` سے `~/.yantrikclaw/` میں منتقل کرتا ہے۔ کنفیگ خود بخود JSON سے TOML میں تبدیل ہو جاتی ہے۔

## سیکیورٹی ڈیفالٹس (DM رسائی)

YantrikClaw حقیقی پیغام رسانی سطحوں سے جڑتا ہے۔ آنے والے DMs کو غیر بھروسہ مند ان پٹ سمجھیں۔
...
- منظوری دیں: `yantrikclaw pairing approve <channel> <code>` (پھر بھیجنے والا مقامی اجازت نامہ میں شامل ہو جاتا ہے)۔
- عوامی آنے والے DMs کے لیے `config.toml` میں واضح opt-in ضروری ہے۔
- خطرناک یا غلط ترتیب شدہ DM پالیسیوں کا پتہ لگانے کے لیے `yantrikclaw doctor` چلائیں۔
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _اہم ترین_  | ہم `openagen/yantrikclaw`، `yantrikclaw.org` یا `yantrikclaw.net` سے **وابستہ نہیں** ہیں۔ `yantrikclaw.org` اور `yantrikclaw.net` ڈومینز فی الحال `openagen/yantrikclaw` فورک کی طرف اشارہ کرتے ہیں، اور وہ ڈومین/ریپوزٹری ہماری سرکاری ویب سائٹ/پروجیکٹ کی نقل کر رہے ہیں۔                                                                                       | ان ذرائع سے معلومات، بائنریز، فنڈ ریزنگ، یا اعلانات پر بھروسہ نہ کریں۔ صرف [یہ ریپوزٹری](https://github.com/yantrikclaw-labs/yantrikclaw) اور ہمارے تصدیق شدہ سوشل اکاؤنٹس استعمال کریں۔                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _اہم_ | ہماری سرکاری ویب سائٹ اب فعال ہے: [yantrikclawlabs.ai](https://yantrikclawlabs.ai)۔ لانچ کی تیاری کے دوران آپ کے صبر کا شکریہ۔ ہم اب بھی نقل کی کوششیں دیکھ رہے ہیں، لہذا YantrikClaw نام کا دعویٰ کرنے والی کسی بھی سرمایہ کاری یا فنڈ ریزنگ سرگرمی میں **شامل نہ ہوں** جب تک کہ یہ ہمارے سرکاری چینلز کے ذریعے شائع نہ ہو۔                            | [یہ ریپوزٹری](https://github.com/yantrikclaw-labs/yantrikclaw) کو واحد سچائی کا ذریعہ استعمال کریں۔ سرکاری اپ ڈیٹس کے لیے [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21)، [Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs)، اور [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) فالو کریں۔ |
...
### ٹیمیں YantrikClaw کیوں چنتی ہیں
...
## بینچ مارک سنیپ شاٹ (YantrikClaw بمقابلہ OpenClaw، قابل تکرار)
...
کم از کم `~/.yantrikclaw/config.toml`:
...
YantrikClaw سبسکرپشن نیٹو auth پروفائلز سپورٹ کرتا ہے (ملٹی اکاؤنٹ، آرام پر خفیہ)۔

- اسٹور فائل: `~/.yantrikclaw/auth-profiles.json`
- خفیہ کاری کلید: `~/.yantrikclaw/.secret_key`
...
Workspace روٹ: `~/.yantrikclaw/workspace/` (config کے ذریعے قابل ترتیب)۔
...
اسکلز: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` یا `SKILL.toml`۔
...
```bash
# Workspace مینجمنٹ
yantrikclaw onboard              # رہنمائی شدہ سیٹ اپ وزرڈ
yantrikclaw status               # daemon/ایجنٹ اسٹیٹس دکھائیں
yantrikclaw doctor               # سسٹم تشخیص چلائیں

# Gateway + daemon
yantrikclaw gateway              # Gateway سرور شروع کریں (127.0.0.1:42617)
yantrikclaw daemon               # مکمل خودمختار رن ٹائم شروع کریں

# ایجنٹ
yantrikclaw agent                # انٹرایکٹو چیٹ موڈ
yantrikclaw agent -m "message"   # واحد پیغام موڈ

# سروس مینجمنٹ
yantrikclaw service install      # OS سروس کے طور پر انسٹال کریں (launchd/systemd)
yantrikclaw service start|stop|restart|status

# چینلز
yantrikclaw channel list         # ترتیب شدہ چینلز کی فہرست
yantrikclaw channel doctor       # چینل صحت چیک کریں
yantrikclaw channel bind-telegram 123456789

# Cron + شیڈولنگ
yantrikclaw cron list            # شیڈولڈ جابز کی فہرست
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# میموری
yantrikclaw memory list          # میموری اندراجات کی فہرست
yantrikclaw memory get <key>     # میموری حاصل کریں
yantrikclaw memory stats         # میموری اعداد و شمار
...
# ہارڈویئر پیری فیرلز
yantrikclaw hardware discover    # منسلک آلات اسکین کریں
yantrikclaw peripheral list      # منسلک پیری فیرلز کی فہرست
yantrikclaw peripheral flash     # آلے پر فرم ویئر فلیش کریں
...
یا اوپر کے مراحل چھوڑیں اور سب کچھ (سسٹم dependencies، Rust، YantrikClaw) ایک کمانڈ میں انسٹال کریں:
...
- رہنمائی شدہ سیٹ اپ کے لیے [YantrikClaw Onboard](#فوری-آغاز) فالو کریں۔
...
YantrikClaw smooth crab 🦀 کے لیے بنایا گیا تھا، ایک تیز اور مؤثر AI اسسٹنٹ۔ Argenis De La Rosa اور کمیونٹی نے بنایا۔

- [yantrikclawlabs.ai](https://yantrikclawlabs.ai)
- [@yantrikclawlabs](https://x.com/yantrikclawlabs)

## YantrikClaw کی حمایت کریں

اگر YantrikClaw آپ کے کام میں مدد کرتا ہے اور آپ جاری ترقی کی حمایت کرنا چاہتے ہیں، تو آپ یہاں عطیہ دے سکتے ہیں:
...
YantrikClaw میں نئے ہیں؟ [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) لیبل والے issues تلاش کریں — شروع کرنے کے طریقے کے لیے [شراکت گائیڈ](CONTRIBUTING.md#first-time-contributors) دیکھیں۔ AI/vibe-coded PRs کا خیرمقدم ہے! 🤖
...
- نیا `Skill` → `~/.yantrikclaw/workspace/skills/<name>/`
...
**یہ YantrikClaw کی واحد سرکاری ریپوزٹری ہے:**

> https://github.com/yantrikclaw-labs/yantrikclaw

کوئی بھی دوسری ریپوزٹری، تنظیم، ڈومین، یا پیکیج جو "YantrikClaw" ہونے کا دعویٰ کرے یا YantrikClaw Labs سے وابستگی کا اشارہ کرے **غیر مجاز ہے اور اس پروجیکٹ سے وابستہ نہیں ہے**۔ معلوم غیر مجاز فورکس [TRADEMARK.md](docs/maintainers/trademark.md) میں درج ہوں گے۔

اگر آپ کو نقل یا ٹریڈ مارک کا غلط استعمال ملے، براہ کرم [issue کھولیں](https://github.com/yantrikclaw-labs/yantrikclaw/issues)۔

---

## لائسنس

YantrikClaw زیادہ سے زیادہ کشادگی اور شراکت دار تحفظ کے لیے دوہری لائسنس یافتہ ہے:
...
**YantrikClaw** نام اور لوگو YantrikClaw Labs کے ٹریڈ مارکس ہیں۔ یہ لائسنس انہیں توثیق یا وابستگی کا اشارہ دینے کے لیے استعمال کرنے کی اجازت نہیں دیتا۔ مجاز اور ممنوع استعمال کے لیے [TRADEMARK.md](docs/maintainers/trademark.md) دیکھیں۔
...
**YantrikClaw** — صفر اوور ہیڈ۔ صفر سمجھوتا۔ کہیں بھی تعینات کریں۔ کچھ بھی تبدیل کریں۔ 🦀
