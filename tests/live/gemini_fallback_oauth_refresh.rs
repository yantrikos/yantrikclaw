...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.yantrikclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.yantrikclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let yantrikclaw_dir = PathBuf::from(home).join(".yantrikclaw");
    let auth_profiles_path = yantrikclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: yantrikclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: yantrikclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.yantrikclaw/auth-profiles.json (with expired token)
    let provider = yantrikclaw::providers::create_provider("gemini", None)?;
