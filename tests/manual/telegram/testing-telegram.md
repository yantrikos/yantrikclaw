...
This guide covers testing the Telegram channel integration for YantrikClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug yantrikclaw channel start
    ```
...
6. **Health check timeout**
    ...
    ```bash
    time yantrikclaw channel doctor
    ```
...
Solution: Check user allowlist
  ...
  4. Run: yantrikclaw onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info yantrikclaw daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time yantrikclaw channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep yantrikclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/yantrikclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace yantrikclaw channel start
...
# Validate config
cat ~/.yantrikclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [YantrikClaw Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/yantrikclaw-labs/yantrikclaw/issues)
