#!/bin/bash
# YantrikClaw Telegram Integration Test Suite
...
if [ -f "target/release/yantrikclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/yantrikclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/yantrikclaw 2>/dev/null || stat -c%s target/release/yantrikclaw)
...
CONFIG_PATH="$HOME/.yantrikclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'yantrikclaw onboard' first"
    fi
else
    warn "No config file found - run 'yantrikclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/yantrikclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
