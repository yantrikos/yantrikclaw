...
<h1 align="center">🦀 YantrikClaw — مساعد الذكاء الاصطناعي الشخصي</h1>
...
> **الإعداد المفضل:** شغّل `yantrikclaw onboard` في طرفيتك. YantrikClaw Onboard يرشدك خطوة بخطوة لإعداد البوابة ومساحة العمل والقنوات والمزود. إنه مسار الإعداد الموصى به ويعمل على macOS وLinux وWindows (عبر WSL2). تثبيت جديد؟ ابدأ هنا: [البدء](#البداية-السريعة)
...
`yantrikclaw onboard` يعمل تلقائيًا بعد التثبيت لتكوين مساحة العمل والمزود.
...
هل تقوم بالترقية؟ شغّل `yantrikclaw doctor` بعد التحديث.
...
يمكن لـ YantrikClaw استيراد مساحة عمل OpenClaw والذاكرة والتكوين الخاص بك:
...
يقوم هذا بترحيل إدخالات الذاكرة وملفات مساحة العمل والتكوين من `~/.openclaw/` إلى `~/.yantrikclaw/`. يتم تحويل التكوين من JSON إلى TOML تلقائيًا.

## إعدادات الأمان الافتراضية (الوصول عبر الرسائل المباشرة)

يتصل YantrikClaw بأسطح المراسلة الحقيقية. تعامل مع الرسائل المباشرة الواردة كمدخلات غير موثوقة.
...
- الموافقة باستخدام: `yantrikclaw pairing approve <channel> <code>` (ثم يُضاف المرسل إلى قائمة السماح المحلية).
- تتطلب الرسائل المباشرة العامة الواردة اشتراكًا صريحًا في `config.toml`.
- شغّل `yantrikclaw doctor` لكشف سياسات الرسائل المباشرة الخطرة أو المُعدة خطأ.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _حرج_ | نحن **غير مرتبطين** بـ `openagen/yantrikclaw` أو `yantrikclaw.org` أو `yantrikclaw.net`. نطاقا `yantrikclaw.org` و`yantrikclaw.net` يشيران حاليًا إلى نسخة `openagen/yantrikclaw` المتفرعة، وهذا النطاق/المستودع ينتحل صفة موقعنا/مشروعنا الرسمي. | لا تثق بالمعلومات أو الملفات الثنائية أو جمع التبرعات أو الإعلانات من تلك المصادر. استخدم فقط [هذا المستودع](https://github.com/yantrikclaw-labs/yantrikclaw) وحساباتنا الاجتماعية الموثقة. |
| 2026-02-21 | _مهم_ | موقعنا الرسمي متاح الآن: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). شكرًا لصبركم أثناء تحضيرنا للإطلاق. ما زلنا نرى محاولات انتحال، لذا **لا** تنضم إلى أي نشاط استثمار أو جمع تبرعات يدعي اسم YantrikClaw ما لم يتم نشره عبر قنواتنا الرسمية. | استخدم [هذا المستودع](https://github.com/yantrikclaw-labs/yantrikclaw) كمصدر الحقيقة الوحيد. تابع [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21) و[Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs) و[Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) للتحديثات الرسمية. |
...
## لقطة المقارنة المرجعية (YantrikClaw مقابل OpenClaw، قابلة للتكرار)
...
> ملاحظات: نتائج YantrikClaw تم قياسها على إصدارات الإنتاج باستخدام `/usr/bin/time -l`. يتطلب OpenClaw بيئة تشغيل Node.js (عادةً ~390 ميجابايت حمل ذاكرة إضافي)، بينما يتطلب NanoBot بيئة تشغيل Python. PicoClaw وYantrikClaw ملفات ثنائية ثابتة. أرقام الرام أعلاه هي ذاكرة وقت التشغيل؛ متطلبات التجميع في وقت البناء أعلى.
...
الحد الأدنى `~/.yantrikclaw/config.toml`:
...
يدعم YantrikClaw ملفات تعريف مصادقة أصلية للاشتراك (متعددة الحسابات، مشفرة عند الراحة).

- ملف التخزين: `~/.yantrikclaw/auth-profiles.json`
- مفتاح التشفير: `~/.yantrikclaw/.secret_key`
...
جذر مساحة العمل: `~/.yantrikclaw/workspace/` (قابل للتكوين عبر التكوين).
...
المهارات: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` أو `SKILL.toml`.
...
أو تخطى الخطوات أعلاه وثبّت كل شيء (تبعيات النظام، Rust، YantrikClaw) بأمر واحد:
...
- اتبع [YantrikClaw Onboard](#البداية-السريعة) للإعداد الموجه.
...
تم بناء YantrikClaw للسلطعون الناعم 🦀، مساعد ذكاء اصطناعي سريع وفعال. بناه Argenis De La Rosa والمجتمع.
...
إذا ساعدك YantrikClaw في عملك وتريد دعم التطوير المستمر، يمكنك التبرع هنا:
...
جديد على YantrikClaw؟ ابحث عن المشكلات المصنفة [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — انظر [دليل المساهمة](CONTRIBUTING.md#first-time-contributors) لمعرفة كيفية البدء. مرحبًا بمساهمات AI/vibe-coded! 🤖
...
- `Skill` جديد → `~/.yantrikclaw/workspace/skills/<name>/`
...
**هذا هو مستودع YantrikClaw الرسمي الوحيد:**

> https://github.com/yantrikclaw-labs/yantrikclaw

أي مستودع أو منظمة أو نطاق أو حزمة أخرى تدعي أنها "YantrikClaw" أو تشير إلى انتمائها لـ YantrikClaw Labs هي **غير مصرح بها وغير مرتبطة بهذا المشروع**. سيتم سرد النسخ المتفرعة غير المصرح بها المعروفة في [TRADEMARK.md](docs/maintainers/trademark.md).

إذا واجهت انتحالًا أو إساءة استخدام للعلامة التجارية، يرجى [فتح مشكلة](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## الترخيص

YantrikClaw مرخص بترخيص مزدوج لأقصى انفتاح وحماية للمساهمين:
...
اسم وشعار **YantrikClaw** هما علامتان تجاريتان لـ YantrikClaw Labs. لا يمنح هذا الترخيص إذنًا لاستخدامهما للإشارة إلى التأييد أو الانتماء. انظر [TRADEMARK.md](docs/maintainers/trademark.md) للاستخدامات المسموحة والمحظورة.
...
**YantrikClaw** — صفر حمل زائد. صفر تنازلات. انشر في أي مكان. بدّل أي شيء. 🦀
