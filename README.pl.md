...
<h1 align="center">🦀 YantrikClaw — Osobisty Asystent AI</h1>
...
> **Zalecana konfiguracja:** uruchom `yantrikclaw onboard` w terminalu. YantrikClaw Onboard prowadzi Cię krok po kroku przez konfigurację gateway, workspace, kanałów i dostawcy. Jest to zalecana ścieżka konfiguracji i działa na macOS, Linux i Windows (przez WSL2). Nowa instalacja? Zacznij tutaj: [Rozpocznij](#szybki-start)
...
`yantrikclaw onboard` uruchamia się automatycznie po instalacji, aby skonfigurować workspace i dostawcę.
...
yantrikclaw gateway                # domyślnie: 127.0.0.1:42617
yantrikclaw gateway --port 0       # losowy port (wzmocnione bezpieczeństwo)
...
Aktualizujesz? Uruchom `yantrikclaw doctor` po aktualizacji.
...
YantrikClaw może zaimportować Twój workspace, pamięć i konfigurację OpenClaw:
...
Migruje wpisy pamięci, pliki workspace i konfigurację z `~/.openclaw/` do `~/.yantrikclaw/`. Konfiguracja jest automatycznie konwertowana z JSON do TOML.

## Domyślne ustawienia bezpieczeństwa (dostęp DM)

YantrikClaw łączy się z prawdziwymi platformami komunikacyjnymi. Traktuj przychodzące DM jako niezaufane dane wejściowe.
...
- Zatwierdź za pomocą: `yantrikclaw pairing approve <channel> <code>` (wtedy nadawca jest dodawany do lokalnej listy dozwolonych).
- Publiczne przychodzące DM wymagają jawnej zgody w `config.toml`.
- Uruchom `yantrikclaw doctor`, aby wykryć ryzykowne lub błędnie skonfigurowane polityki DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Krytyczny_  | **Nie jesteśmy powiązani** z `openagen/yantrikclaw`, `yantrikclaw.org` ani `yantrikclaw.net`. Domeny `yantrikclaw.org` i `yantrikclaw.net` obecnie kierują do forka `openagen/yantrikclaw`, a ta domena/repozytorium podszywają się pod naszą oficjalną stronę/projekt.                                                                                       | Nie ufaj informacjom, plikom binarnym, zbiórkom funduszy ani ogłoszeniom z tych źródeł. Używaj wyłącznie [tego repozytorium](https://github.com/yantrikclaw-labs/yantrikclaw) i naszych zweryfikowanych kont społecznościowych.                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Ważny_ | Nasza oficjalna strona internetowa jest teraz dostępna: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). Dziękujemy za cierpliwość podczas przygotowywania premiery. Nadal obserwujemy próby podszywania się, więc **nie** dołączaj do żadnych inwestycji ani zbiórek funduszy pod nazwą YantrikClaw, chyba że zostały opublikowane przez nasze oficjalne kanały.                            | Używaj [tego repozytorium](https://github.com/yantrikclaw-labs/yantrikclaw) jako jedynego źródła prawdy. Śledź [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Grupa)](https://www.facebook.com/groups/yantrikclawlabs) i [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) po oficjalne aktualizacje. |
...
### Dlaczego zespoły wybierają YantrikClaw
...
## Porównanie wydajności (YantrikClaw vs OpenClaw, odtwarzalne)
...
> Uwagi: Wyniki YantrikClaw są mierzone na buildach release przy użyciu `/usr/bin/time -l`. OpenClaw wymaga środowiska Node.js (typowo ~390MB dodatkowego narzutu pamięci), natomiast NanoBot wymaga środowiska Python. PicoClaw i YantrikClaw to statyczne pliki binarne. Powyższe wartości RAM dotyczą pamięci runtime; wymagania kompilacji są wyższe.
...
Minimalna `~/.yantrikclaw/config.toml`:
...
YantrikClaw obsługuje natywne profile autoryzacyjne subskrypcji (wiele kont, szyfrowanie w spoczynku).

- Plik przechowywania: `~/.yantrikclaw/auth-profiles.json`
- Klucz szyfrowania: `~/.yantrikclaw/.secret_key`
...
Katalog główny workspace: `~/.yantrikclaw/workspace/` (konfigurowalne przez config).
...
Umiejętności: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` lub `SKILL.toml`.
...
yantrikclaw onboard              # Kreator konfiguracji z przewodnikiem
yantrikclaw status               # Pokaż status demona/agenta
yantrikclaw doctor               # Uruchom diagnostykę systemu

# Gateway + demon
yantrikclaw gateway              # Uruchom serwer gateway (127.0.0.1:42617)
yantrikclaw daemon               # Uruchom pełne autonomiczne środowisko

# Agent
yantrikclaw agent                # Tryb interaktywnego czatu
yantrikclaw agent -m "message"   # Tryb pojedynczej wiadomości

# Zarządzanie usługami
yantrikclaw service install      # Zainstaluj jako usługę OS (launchd/systemd)
yantrikclaw service start|stop|restart|status

# Kanały
yantrikclaw channel list         # Lista skonfigurowanych kanałów
yantrikclaw channel doctor       # Sprawdź zdrowie kanałów
yantrikclaw channel bind-telegram 123456789

# Cron + planowanie
yantrikclaw cron list            # Lista zaplanowanych zadań
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# Pamięć
yantrikclaw memory list          # Lista wpisów pamięci
yantrikclaw memory get <key>     # Pobierz wspomnienie
yantrikclaw memory stats         # Statystyki pamięci
...
# Peryferia sprzętowe
yantrikclaw hardware discover    # Skanuj podłączone urządzenia
yantrikclaw peripheral list      # Lista podłączonych peryferiów
yantrikclaw peripheral flash     # Flash firmware na urządzenie
...
Lub pomiń powyższe kroki i zainstaluj wszystko (zależności systemowe, Rust, YantrikClaw) jednym poleceniem:
...
- Postępuj zgodnie z [YantrikClaw Onboard](#szybki-start) dla konfiguracji z przewodnikiem.
...
YantrikClaw został zbudowany dla smooth crab 🦀, szybkiego i wydajnego asystenta AI. Stworzony przez Argenisa De La Rosę i społeczność.

- [yantrikclawlabs.ai](https://yantrikclawlabs.ai)
- [@yantrikclawlabs](https://x.com/yantrikclawlabs)

## Wesprzyj YantrikClaw

Jeśli YantrikClaw pomaga w Twojej pracy i chcesz wesprzeć dalszy rozwój, możesz przekazać darowiznę tutaj:
...
Nowy w YantrikClaw? Szukaj issues oznaczonych [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — zobacz nasz [Przewodnik kontrybutora](CONTRIBUTING.md#first-time-contributors), aby dowiedzieć się jak zacząć. PR-y z AI/vibe-coded mile widziane! 🤖
...
- Nowy `Skill` → `~/.yantrikclaw/workspace/skills/<name>/`
...
**To jest jedyne oficjalne repozytorium YantrikClaw:**

> https://github.com/yantrikclaw-labs/yantrikclaw

Każde inne repozytorium, organizacja, domena lub pakiet twierdzący, że jest "YantrikClaw" lub sugerujący powiązanie z YantrikClaw Labs jest **nieautoryzowany i niepowiązany z tym projektem**. Znane nieautoryzowane forki będą wymienione w [TRADEMARK.md](docs/maintainers/trademark.md).

Jeśli napotkasz podszywanie się lub nadużycie znaku towarowego, proszę [otwórz zgłoszenie](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## Licencja

YantrikClaw jest podwójnie licencjonowany dla maksymalnej otwartości i ochrony kontrybutorów:
...
Nazwa **YantrikClaw** i logo są znakami towarowymi YantrikClaw Labs. Ta licencja nie udziela pozwolenia na ich używanie w celu sugerowania poparcia lub powiązania. Zobacz [TRADEMARK.md](docs/maintainers/trademark.md) po dozwolone i zabronione użycia.
...
**YantrikClaw** — Zero narzutu. Zero kompromisów. Wdrażaj wszędzie. Wymieniaj wszystko. 🦀
