# YantrikClaw Development Environment

A fully containerized development sandbox for YantrikClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`yantrikclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `yantrikclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/yantrikclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/yantrikclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.yantrikclaw` (mounted to `/yantrikclaw-data/.yantrikclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.yantrikclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `yantrikclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=yantrikclaw-target`) to speed repeat local image builds.
