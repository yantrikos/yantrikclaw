...
ASSET_URL="https://github.com/yantrikclaw-labs/yantrikclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t yantrikclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains yantrikclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^yantrikclaw$"; then
  pass "Archive contains 'yantrikclaw' binary"
else
  fail "Archive does not contain 'yantrikclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/yantrikclaw-labs/yantrikclaw/releases/download/${TAG}/SHA256SUMS"
