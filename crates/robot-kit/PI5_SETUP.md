...
Complete guide to setting up a YantrikClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.yantrikclaw/models
cp models/ggml-base.bin ~/.yantrikclaw/models/
```
...
mkdir -p ~/.yantrikclaw/models/piper
cd ~/.yantrikclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.yantrikclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build YantrikClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/yantrikclaw-labs/yantrikclaw
cd yantrikclaw

# Build robot kit
cargo build --release -p yantrikclaw-robot-kit

# Build main yantrikclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.yantrikclaw
nano ~/.yantrikclaw/robot.toml
```

```toml
# ~/.yantrikclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.yantrikclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/yantrikclaw_sensors.fifo"
...
### Start YantrikClaw Agent

```bash
# Configure YantrikClaw to use robot tools
cat > ~/.yantrikclaw/config.toml << 'EOF'
...
cp ~/yantrikclaw/crates/robot-kit/SOUL.md ~/.yantrikclaw/workspace/

# Start agent
./target/release/yantrikclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/yantrikclaw_sensors.fifo ]; then
    mkfifo /tmp/yantrikclaw_sensors.fifo
fi
...
# Start yantrikclaw
cd ~/yantrikclaw
./target/release/yantrikclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/yantrikclaw-robot.service
sudo tee /etc/systemd/system/yantrikclaw-robot.service << 'EOF'
[Unit]
Description=YantrikClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/yantrikclaw
...
sudo systemctl enable yantrikclaw-robot
sudo systemctl start yantrikclaw-robot

# Check status
sudo systemctl status yantrikclaw-robot
journalctl -u yantrikclaw-robot -f  # View logs
```
