...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/yantrikclaw-labs/yantrikclaw/security/advisories/new)
...
YantrikClaw implements defense-in-depth security:
...
YantrikClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t yantrikclaw .
docker inspect --format='{{.Config.User}}' yantrikclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace yantrikclaw gateway
```
