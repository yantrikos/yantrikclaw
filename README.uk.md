...
<h1 align="center">🦀 YantrikClaw — Персональний AI-Асистент</h1>
...
`yantrikclaw onboard` запускається автоматично після встановлення для налаштування вашого робочого простору та провайдера.
...
yantrikclaw gateway                # за замовчуванням: 127.0.0.1:42617
yantrikclaw gateway --port 0       # випадковий порт (посилена безпека)
...
Оновлюєтесь? Виконайте `yantrikclaw doctor` після оновлення.
...
YantrikClaw може імпортувати ваш робочий простір, пам'ять та конфігурацію OpenClaw:
...
Це мігрує ваші записи пам'яті, файли робочого простору та конфігурацію з `~/.openclaw/` до `~/.yantrikclaw/`. Конфігурація автоматично конвертується з JSON у TOML.

## Стандартні налаштування безпеки (доступ через DM)

YantrikClaw підключається до реальних платформ обміну повідомленнями. Розглядайте вхідні DM як ненадійний ввід.
...
- Підтвердіть за допомогою: `yantrikclaw pairing approve <channel> <code>` (після чого відправник додається до локального списку дозволених).
- Публічні вхідні DM вимагають явного увімкнення в `config.toml`.
- Виконайте `yantrikclaw doctor` для виявлення ризикованих або неправильно налаштованих політик DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Критичний_ | Ми **не пов'язані** з `openagen/yantrikclaw`, `yantrikclaw.org` або `yantrikclaw.net`. Домени `yantrikclaw.org` та `yantrikclaw.net` наразі вказують на форк `openagen/yantrikclaw`, і цей домен/репозиторій видають себе за наш офіційний вебсайт/проєкт. | Не довіряйте інформації, бінарним файлам, збору коштів або оголошенням з цих джерел. Використовуйте лише [цей репозиторій](https://github.com/yantrikclaw-labs/yantrikclaw) та наші верифіковані соціальні акаунти. |
| 2026-02-21 | _Важливий_ | Наш офіційний вебсайт тепер доступний: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). Дякуємо за терпіння, поки ми готували запуск. Ми все ще бачимо спроби імітації, тому **не** приєднуйтесь до будь-якої інвестиційної або збіркової діяльності, що використовує назву YantrikClaw, якщо вона не опублікована через наші офіційні канали. | Використовуйте [цей репозиторій](https://github.com/yantrikclaw-labs/yantrikclaw) як єдине джерело істини. Слідкуйте за [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (Group)](https://www.facebook.com/groups/yantrikclawlabs) та [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) для офіційних оновлень. |
...
### Чому команди обирають YantrikClaw
...
## Порівняльний бенчмарк (YantrikClaw проти OpenClaw, відтворюваний)
...
> Примітки: результати YantrikClaw виміряні на релізних збірках за допомогою `/usr/bin/time -l`. OpenClaw вимагає середовище Node.js (зазвичай ~390MB додаткових накладних витрат пам'яті), тоді як NanoBot вимагає середовище Python. PicoClaw і YantrikClaw — це статичні бінарні файли. Наведені цифри RAM — це пам'ять часу виконання; вимоги до компіляції вищі.
...
Мінімальний `~/.yantrikclaw/config.toml`:
...
YantrikClaw підтримує профілі авторизації на основі підписки (мультиакаунт, шифрування в стані спокою).

- Файл сховища: `~/.yantrikclaw/auth-profiles.json`
- Ключ шифрування: `~/.yantrikclaw/.secret_key`
...
Корінь робочого простору: `~/.yantrikclaw/workspace/` (налаштовується через конфігурацію).
...
Навички: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` або `SKILL.toml`.
...
yantrikclaw onboard              # Покроковий майстер налаштування
yantrikclaw status               # Показати стан демона/агента
yantrikclaw doctor               # Запустити діагностику системи

# Gateway + демон
yantrikclaw gateway              # Запустити сервер gateway (127.0.0.1:42617)
yantrikclaw daemon               # Запустити повне автономне середовище

# Агент
yantrikclaw agent                # Інтерактивний режим чату
yantrikclaw agent -m "message"   # Режим одного повідомлення

# Керування сервісом
yantrikclaw service install      # Встановити як системний сервіс (launchd/systemd)
yantrikclaw service start|stop|restart|status

# Канали
yantrikclaw channel list         # Список налаштованих каналів
yantrikclaw channel doctor       # Перевірка стану каналів
yantrikclaw channel bind-telegram 123456789

# Cron + планування
yantrikclaw cron list            # Список запланованих завдань
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# Пам'ять
yantrikclaw memory list          # Список записів пам'яті
yantrikclaw memory get <key>     # Отримати запис пам'яті
yantrikclaw memory stats         # Статистика пам'яті
...
yantrikclaw hardware discover    # Сканування підключених пристроїв
yantrikclaw peripheral list      # Список підключених периферійних пристроїв
yantrikclaw peripheral flash     # Прошивка пристрою
...
Або пропустіть кроки вище і встановіть все (системні залежності, Rust, YantrikClaw) однією командою:
...
- Слідуйте [YantrikClaw Onboard](#швидкий-старт-tldr) для покрокового налаштування.
...
YantrikClaw створений для smooth crab 🦀, швидкого та ефективного AI-асистента. Створений Argenis De La Rosa та спільнотою.

- [yantrikclawlabs.ai](https://yantrikclawlabs.ai)
- [@yantrikclawlabs](https://x.com/yantrikclawlabs)

## Підтримайте YantrikClaw

Якщо YantrikClaw допомагає вашій роботі і ви хочете підтримати подальшу розробку, ви можете зробити пожертву тут:
...
Новачок у YantrikClaw? Шукайте завдання з міткою [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — дивіться наш [Посібник з внеску](CONTRIBUTING.md#first-time-contributors) для початку. PR з AI-допомогою вітаються!
...
- Новий `Skill` → `~/.yantrikclaw/workspace/skills/<name>/`
...
**Це єдиний офіційний репозиторій YantrikClaw:**

> https://github.com/yantrikclaw-labs/yantrikclaw

Будь-який інший репозиторій, організація, домен або пакет, що претендує на назву "YantrikClaw" або натякає на зв'язок з YantrikClaw Labs, є **неавторизованим і не пов'язаним з цим проєктом**. Відомі неавторизовані форки перелічені в [TRADEMARK.md](docs/maintainers/trademark.md).

Якщо ви зіткнулися з імітацією або зловживанням торговою маркою, будь ласка, [створіть issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## Ліцензія

YantrikClaw має подвійну ліцензію для максимальної відкритості та захисту учасників:
...
Назва та логотип **YantrikClaw** є торговими марками YantrikClaw Labs. Ця ліцензія не надає дозволу використовувати їх для підтвердження або зв'язку. Дивіться [TRADEMARK.md](docs/maintainers/trademark.md) для дозволених та заборонених використань.
...
**YantrikClaw** — Нуль накладних витрат. Нуль компромісів. Розгортайте будь-де. Замінюйте будь-що. 🦀
