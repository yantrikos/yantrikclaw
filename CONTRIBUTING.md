# Contributing to YantrikClaw

Thanks for your interest in contributing to YantrikClaw! This guide will help you get started.
...
**Background:** YantrikClaw previously used `main` in some documentation and scripts, which caused 404 errors, broken CI refs, and contributor confusion (see [#2929](https://github.com/yantrikclaw-labs/yantrikclaw/issues/2929), [#3061](https://github.com/yantrikclaw-labs/yantrikclaw/issues/3061), [#3194](https://github.com/yantrikclaw-labs/yantrikclaw/pull/3194)). As of March 2026, all references have been corrected, stale branches cleaned up, and the `main` branch permanently deleted.
...
1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** YantrikClaw uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
YantrikClaw supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.yantrikclaw/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.yantrikclaw/.secret_key` with restricted permissions
    - Use `yantrikclaw onboard` for guided setup
...
3. Generic env vars (`YANTRIKCLAW_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `YANTRIKCLAW_PROVIDER` / `PROVIDER`
- `YANTRIKCLAW_MODEL`
...
- `~/.yantrikclaw/.secret_key` or similar key files
...
- If identity-like wording is required, use YantrikClaw-centric labels only (for example: `YantrikClawAgent`, `YantrikClawOperator`, `yantrikclaw_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use YantrikClaw/project-native roles instead of personal or real-world identities.
...
YantrikClaw's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use YantrikClaw-native identifiers only (`YantrikClawAgent`, `yantrikclaw_user`, `yantrikclaw_node`).
...
- **Good identity-like label**: `YantrikClawAgent`, `yantrikclaw_user`, `yantrikclaw_node`
...
- [ ] Any required identity-like wording uses YantrikClaw/project-native labels only
