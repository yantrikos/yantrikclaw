...
<h1 align="center">🦀 YantrikClaw — Trợ lý AI Cá nhân</h1>
...
> **Cài đặt khuyến nghị:** chạy `yantrikclaw onboard` trong terminal. YantrikClaw Onboard hướng dẫn bạn từng bước thiết lập gateway, workspace, kênh và provider. Đây là đường dẫn cài đặt được khuyến nghị và hoạt động trên macOS, Linux, và Windows (qua WSL2). Cài đặt mới? Bắt đầu tại đây: [Bắt đầu](#bắt-đầu-nhanh-tldr)
...
`yantrikclaw onboard` tự động chạy sau khi cài đặt để cấu hình workspace và provider.
...
yantrikclaw gateway                # mặc định: 127.0.0.1:42617
yantrikclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
Đang nâng cấp? Chạy `yantrikclaw doctor` sau khi cập nhật.
...
YantrikClaw có thể nhập workspace, bộ nhớ và cấu hình OpenClaw của bạn:
...
Thao tác này chuyển đổi các mục bộ nhớ, file workspace và cấu hình từ `~/.openclaw/` sang `~/.yantrikclaw/`. Cấu hình được tự động chuyển từ JSON sang TOML.

## Mặc định bảo mật (truy cập DM)

YantrikClaw kết nối với các dịch vụ nhắn tin thực. Xem DM đến như đầu vào không đáng tin cậy.
...
- Phê duyệt bằng: `yantrikclaw pairing approve <channel> <code>` (người gửi được thêm vào danh sách cho phép cục bộ).
- DM đến công khai yêu cầu opt-in rõ ràng trong `config.toml`.
- Chạy `yantrikclaw doctor` để phát hiện chính sách DM nguy hiểm hoặc cấu hình sai.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không liên kết** với `openagen/yantrikclaw`, `yantrikclaw.org` hay `yantrikclaw.net`. Các tên miền `yantrikclaw.org` và `yantrikclaw.net` hiện đang trỏ đến fork `openagen/yantrikclaw`, và các tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng [repository này](https://github.com/yantrikclaw-labs/yantrikclaw) và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
| 2026-02-21 | _Quan trọng_ | Website chính thức của chúng tôi đã ra mắt: [yantrikclawlabs.ai](https://yantrikclawlabs.ai). Cảm ơn đã kiên nhẫn chờ đợi. Chúng tôi vẫn phát hiện các nỗ lực mạo danh, vì vậy **không** tham gia bất kỳ hoạt động đầu tư hoặc gây quỹ nào nhân danh YantrikClaw trừ khi được công bố qua các kênh chính thức. | Sử dụng [repository này](https://github.com/yantrikclaw-labs/yantrikclaw) làm nguồn thông tin duy nhất đáng tin cậy. Theo dõi [X (@yantrikclawlabs)](https://x.com/yantrikclawlabs?s=21), [Facebook (nhóm)](https://www.facebook.com/groups/yantrikclawlabs), và [Reddit (r/yantrikclawlabs)](https://www.reddit.com/r/yantrikclawlabs/) để nhận cập nhật chính thức. |
...
### Vì sao các team chọn YantrikClaw
...
## So sánh hiệu năng (YantrikClaw vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả YantrikClaw được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), NanoBot yêu cầu runtime Python. PicoClaw và YantrikClaw là static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build cao hơn.
...
Tối thiểu `~/.yantrikclaw/config.toml`:
...
YantrikClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.yantrikclaw/auth-profiles.json`
- Khóa mã hóa: `~/.yantrikclaw/.secret_key`
...
Thư mục gốc workspace: `~/.yantrikclaw/workspace/` (có thể cấu hình qua config).
...
Skill: `~/.yantrikclaw/workspace/skills/<skill>/SKILL.md` hoặc `SKILL.toml`.
...
```bash
# Quản lý workspace
yantrikclaw onboard              # Trình hướng dẫn cài đặt
yantrikclaw status               # Hiển thị trạng thái daemon/agent
yantrikclaw doctor               # Chạy chẩn đoán hệ thống

# Gateway + daemon
yantrikclaw gateway              # Khởi động gateway server (127.0.0.1:42617)
yantrikclaw daemon               # Khởi động runtime tự trị đầy đủ

# Agent
yantrikclaw agent                # Chế độ chat tương tác
yantrikclaw agent -m "message"   # Chế độ tin nhắn đơn

# Quản lý dịch vụ
yantrikclaw service install      # Cài đặt làm dịch vụ OS (launchd/systemd)
yantrikclaw service start|stop|restart|status

# Kênh
yantrikclaw channel list         # Liệt kê kênh đã cấu hình
yantrikclaw channel doctor       # Kiểm tra sức khỏe kênh
yantrikclaw channel bind-telegram 123456789

# Cron + lên lịch
yantrikclaw cron list            # Liệt kê tác vụ đã lên lịch
yantrikclaw cron add "*/5 * * * *" --prompt "Check system health"
yantrikclaw cron remove <id>

# Bộ nhớ
yantrikclaw memory list          # Liệt kê mục bộ nhớ
yantrikclaw memory get <key>     # Truy xuất bộ nhớ
yantrikclaw memory stats         # Thống kê bộ nhớ
...
yantrikclaw hardware discover    # Quét thiết bị đã kết nối
yantrikclaw peripheral list      # Liệt kê thiết bị ngoại vi đã kết nối
yantrikclaw peripheral flash     # Flash firmware vào thiết bị
...
Hoặc bỏ qua các bước trên và cài hết mọi thứ (system deps, Rust, YantrikClaw) bằng một lệnh:
...
- Theo [YantrikClaw Onboard](#bắt-đầu-nhanh-tldr) để cài đặt có hướng dẫn.
...
YantrikClaw được xây dựng cho smooth crab 🦀, một trợ lý AI nhanh và hiệu quả. Được xây dựng bởi Argenis De La Rosa và cộng đồng.
...
Nếu YantrikClaw giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển, bạn có thể quyên góp tại đây:
...
Mới với YantrikClaw? Tìm các issue có nhãn [`good first issue`](https://github.com/yantrikclaw-labs/yantrikclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — xem [Hướng dẫn đóng góp](CONTRIBUTING.md#first-time-contributors) để bắt đầu. PR AI/vibe-coded đều được chào đón! 🤖
...
- `Skill` mới → `~/.yantrikclaw/workspace/skills/<name>/`
...
**Đây là repository YantrikClaw chính thức duy nhất:**

> https://github.com/yantrikclaw-labs/yantrikclaw

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "YantrikClaw" hoặc ngụ ý liên kết với YantrikClaw Labs đều **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](docs/maintainers/trademark.md).

Nếu bạn phát hiện mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/yantrikclaw-labs/yantrikclaw/issues).

---

## Giấy phép

YantrikClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **YantrikClaw** và logo là nhãn hiệu của YantrikClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](docs/maintainers/trademark.md) để biết các sử dụng được phép và bị cấm.
...
**YantrikClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
